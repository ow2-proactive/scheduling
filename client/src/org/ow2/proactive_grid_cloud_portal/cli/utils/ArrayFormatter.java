...
/**
 * ObjectArrayFormatter is used to format (as string) an object in ordered
 * column.
 * 
...
public class ArrayFormatter {
    ...
    /**
     * Set the title value to the given title value. Must have the same number
     * of elements than each added lines.
     * 
     * @param title
     *            the title to set
     */
    ...
    /**
     * Add the given lines value to existing lines. Must have the same number of
     * elements than the titles.
     * 
     * @param lines
     *            the lines to add
     */
    ...
    /**
     * Add a empty line to the list of lines. This will be printed as a newline.
     */
    ...
    /**
     * Set the space value to the given space value. It represent the space
     * between each column.
     * 
     * @param space
     *            the space to set
    ...
    public String getAsString() {
        ...
        for (List<String> l : lines) {
            if (l != null) {
                int i = 0;
                for (String s : l) {
                    columnLengths[i] = (s.length() > columnLengths[i]) ? s
                            .length() : columnLengths[i];
                    i++;
                }
            }
        }
        // make the maxColumnLength the top limit of the int array and add the
        // separator number of spaces
        for (int i = 0; i < columnLengths.length; i++) {
        ...
        for (int i = 0; i < title.size(); i++) {
            sb.append(String.format(" %1$-" + columnLengths[i] + "s",
                    cutNchar(title.get(i), maxColumnLength)));
        }
        ...
        for (List<String> l : lines) {
            if (l == null) {
                ...
                for (int i = 0; i < l.size(); i++) {
                    sb.append(String.format(" %1$-" + columnLengths[i] + "s",
                            cutNchar(l.get(i), maxColumnLength)));
                }
                sb.append(System.getProperty("line.separator"));
            }
        }
     ...
     * @return a string that is the same as the given one if str.length() is
     *         lesser or equals to nbChar, otherwise a shortcut string endind
     *         with '...'
     */
