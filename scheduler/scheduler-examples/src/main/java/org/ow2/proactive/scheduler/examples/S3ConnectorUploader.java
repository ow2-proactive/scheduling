...
public class S3ConnectorUploader extends JavaExecutable {
    ...
    private void uploadDir(String dirPath, String bucketName, String keyPrefix, boolean recursive, boolean pause,
            AmazonS3 s3Client) {
        getOut().println("directory: " + dirPath + (recursive ? " (recursive)" : "") + (pause ? " (pause)" : ""));
     ...
     */
    private Bucket getBucket(String bucket_name, AmazonS3 s3) {
     ...
     * @param s3
     * @return
     * @throws AmazonS3Exception If it is not possible to create the bucket.
     */
    private Bucket createBucketIfNotExists(String bucketName, AmazonS3 s3) throws AmazonS3Exception {
        Bucket b;
