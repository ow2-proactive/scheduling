...
<<<<<<< HEAD
	...
	public synchronized void update(TaskInfo info) {
		...
		try {
			...
			throw new IllegalArgumentException(
					"This task info is not applicable in this job. (task id '" + info.getTaskId() + "' not found)");
		}
	}
	...
	public void startTask(InternalTask td) {
		...
		td.setExecutionHostName(
				td.getExecuterInformation().getHostName() + " (" + td.getExecuterInformation().getNodeName() + ")");
	}
	...
	public ChangedTasksInfo terminateTask(boolean errorOccurred, TaskId taskId, SchedulerStateUpdate frontend,
		...
		if (action != null) {
			...
			case LOOP: {
				 ...
				 */
			case IF: {
				didAction = terminateIfTaskHandler.terminateIfTask(action, initiator, changesInfo, frontend, descriptor,
						taskId);
				break;
			}
				 ...
				 */
			case REPLICATE: {
				didAction = terminateReplicateTaskHandler.terminateReplicateTask(action, initiator, changesInfo,
						frontend, taskId);
				break;
				...
				/*
				 * CONTINUE action : - continue taskflow as if no action was
				 * provided
				 */
			case CONTINUE:
...
=======
    ...
    public ChangedTasksInfo terminateTask(boolean errorOccurred, TaskId taskId,
        ...
        if (action != null) {
            ...
            switch (action.getType()) {
                 ...
                 */
                case LOOP: {
                    didAction = terminateLoopTask(didAction, action, initiator, changesInfo, frontend);
                    break;
                }
                     ...
                     */
                case IF: {
                    didAction = terminateIfTask(action, initiator, changesInfo, frontend, descriptor, taskId);
                    break;
                }
                     ...
                     */
                case REPLICATE: {
                    didAction = terminateReplicateTask(action, initiator, changesInfo, frontend, taskId);
                    break;
    ...
    private boolean terminateLoopTask(boolean didAction, FlowAction action, InternalTask initiator, 
    ...
    private boolean terminateIfTask(FlowAction action, InternalTask initiator, 
			ChangedTasksInfo changesInfo, SchedulerStateUpdate frontend, InternalTask descriptor, TaskId taskId){
    ...
    private boolean terminateReplicateTask(FlowAction action, InternalTask initiator, 
