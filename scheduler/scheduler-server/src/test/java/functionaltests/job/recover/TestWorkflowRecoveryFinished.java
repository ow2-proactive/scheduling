 ...
 */
public class TestWorkflowRecoveryFinished extends SchedulerFunctionalTest {
    ...
    @Test
    public void runWorkflowRecoveryFinished() throws Throwable {
        ...
        for (Entry<Integer, JobId> job : jobs.entrySet()) {
            schedulerHelper.waitForEventJobFinished(job.getValue());
            log("Job finished " + job.getValue());
        }

        log("Crashing Scheduler...");
        ...
        for (Entry<Integer, JobId> job : jobs.entrySet()) {
            JobResult results = schedulerHelper.getJobResult(job.getValue());
