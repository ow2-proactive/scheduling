 ...
 */
package schedulerserverfunctionaltests.jmx;
...
public final class SchedulerJMXTest extends SchedulerFunctionalTestWithRestart {
    ...
    public void action() throws Exception {
...

        {
            ...
            Assert.assertFalse("The jmxRmiServiceURL and jmxRoServiceURL must not be equal",
        ...
        {
            SchedulerTHelper.log("Test invalid JMX auth with null login/password creds (expect SecurityException)");
        ...
        // Tests as user over RMI
        {
            ...
            final String[] attributesToCheck = new String[] { "Status", "TotalJobsCount", "FinishedJobsCount",
                                                              "TotalTasksCount", "FinishedTasksCount" };
        ...
        // Test simultaneous RMI and RO connections
        {
            ...
            Assert.assertFalse("In case of simultaneous RMI and RO JMX connections the connectors must not provide the same connection ids",
                               jmxRmiConnector.getConnectionId().equals(jmxRoConnector1.getConnectionId()));

            SchedulerTHelper.log("Test JMX-RO connection unicity (two connections over RO must not have the same id)");
            final JMXConnector jmxRoConnector2 = JMXConnectorFactory.connect(jmxRoServiceURL, env);
            Assert.assertFalse("In case of multiple RO JMX connections the connectors must not provide the same connection ids",
