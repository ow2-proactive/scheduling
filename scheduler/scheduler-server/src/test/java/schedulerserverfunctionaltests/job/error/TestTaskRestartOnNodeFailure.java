 ...
 */
package schedulerserverfunctionaltests.job.error;
...
import schedulerserverfunctionaltests.utils.SchedulerFunctionalTestWithCustomConfigAndRestart;
...
public class TestTaskRestartOnNodeFailure extends SchedulerFunctionalTestWithCustomConfigAndRestart {
    ...
    private void testTaskKillNode(FileLock fileLock, boolean waitBeforeKill) throws Exception {
        ...
        SchedulerTHelper.log("Submit job");
        final JobId jobId = schedulerHelper.submitJob(createJob(fileLockPath.toString()));

        SchedulerTHelper.log("Wait when node becomes busy");
        RMNodeEvent event;
        do {
        ...
        SchedulerTHelper.log("Wait when task starts");
         ...
         */
        if (waitBeforeKill) {
            SchedulerTHelper.log("Wait some time");
            Thread.sleep(5000);
        }

        SchedulerTHelper.log("Stop task node process (node " + nodeToKill.getNode().getNodeInformation().getURL() + ")");
        nodeToKill.kill();

        TestNode newNode = startNode();

        SchedulerTHelper.log("Let task finish");
        fileLock.unlock();

        SchedulerTHelper.log("Wait when job finish");
        ...
        SchedulerTHelper.log("Check job result");
    ...
    private TestNode startNode() throws Exception {
        ...
        SchedulerTHelper.log("Start new node: node-" + nodeNumber);
