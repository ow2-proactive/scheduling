...
public class UserFolderValidator implements Validator<String> {
    ...
    public String validate(String parameterValue, ModelValidatorContext context) throws ValidationException {
        ...
        if (StringUtils.isBlank(parameterValue)) {
            throw new ValidationException("Please provide a valid folder path in the user space as the variable value.");
        }
        try {
            if (!context.getSpace().checkFileExists(USERSPACE_NAME, parameterValue)) {
                throw new ValidationException(String.format("Could not find the folder path [%s] in the user data space. Please add the folder into the user data space or change the variable value to a valid path.",
                                                            parameterValue));
            }
            if (!context.getSpace().isFolder(USERSPACE_NAME, parameterValue)) {
                throw new ValidationException(String.format("The file path [%s] in the user data space is not a folder. Please change the variable value to a valid path of a folder in the user data space.",
                                                            parameterValue));
            }
