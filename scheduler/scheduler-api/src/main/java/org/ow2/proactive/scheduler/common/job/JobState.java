...
public abstract class JobState extends Job implements Comparable<JobState> {
     ...
     * @param removeIndexes if true, the index part of the tag is replaced by '<index>' if the tag is generated by a LOOP or REPLICATE
     * @return the list of tags.
     */
    public List<String> getTags(boolean removeIndexes){
        Set<String> result = new HashSet<>();
        String tag = null;
        for(TaskState task: this.getTasks()){
            tag = task.getTag();
            if(tag != null) {
                if(removeIndexes && (tag.contains("LOOP") || tag.contains("REPLICATE"))){
                    tag = tag.substring(0, tag.lastIndexOf("-")) + "-<index>";
                }
                result.add(tag);
            }
        }
     ...
     * @return the list of tags.
     */
    public List<String> getTags(String prefix, boolean removeIndexes){
        Set<String> result = new HashSet<>();
        String tag = null;
        for(TaskState task: this.getTasks()){
            tag = task.getTag();
