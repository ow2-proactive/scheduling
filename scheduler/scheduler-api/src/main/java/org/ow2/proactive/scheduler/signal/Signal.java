...
import org.ow2.proactive.scheduler.synchronization.ClosureEvaluationException;
...
/**
 * Signal provides a persistent <i>key/value store</i> API, similar to the {@link Map Map} interface.
 *
 ...
 * Signal provides an API to create or delete channel and decide if the channel should be persisted on disk (thus available on scheduler restart) or kept in memory only.</p>
 *
 * <p>Each map operation is performed on a given <i>channel</i>, with the channel name provided as parameter.</p>
 *
 * <p>The Signal API provides also additional map operations such as {@link #conditionalCompute conditionalCompute} to perform conditional operations based on the result of a {@link java.util.function.BiPredicate}, or wait operations such as {@link #waitUntil(String, String, String)} to block the caller until a given predicate is met.</p>
 *
 * <p>These <i>wait operations</i> allow the API to be used for synchronization and not only as a key/value store. The <i>Signal API</i> is notably used to synchronize parallel ProActive Tasks.</p>
 *
 ...
 *
 * <p>For example, to write a groovy closure which match a Java {@link BiFunction}:</p>
 *
 * <pre> {@code
 * "{ k, x -> x + 1 }"
 * }</pre>
...
public interface Signal extends Serializable {
     ...
     *
     * The action is passed as a String containing a groovy closure definition which matches the {@link BiConsumer} specification.
     * 
     ...
     *
     * The mapping function is passed as a String containing a groovy closure definition which matches the {@link BiFunction} specification.
     *
     * The first parameter receives the key, and the second parameter receives the binding associated with this key.
     *
     * For example:
     ...
     *
     * The function is passed as a String containing a groovy closure definition which matches the {@link Function} specification.
     *
     ...
     *
     * The function is passed as a String containing a groovy closure definition which matches the {@link BiFunction} specification.
     *
     * The first parameter receives the key, and the second parameter receives the binding associated with this key.
     *
     * For example:
     ...
     *
     * Both predicate and function are passed as String containing a groovy closure definition which matches the {@link java.util.function.BiPredicate} and {@link BiFunction} specifications.
     * The first parameter receives the key, and the second parameter receives the binding associated with this key.
     *
     ...
     *
     * Both predicate and functions are passed as String containing a groovy closure definition which matches the {@link java.util.function.BiPredicate} and {@link BiFunction} specifications.
     * The first parameter receives the key, and the second parameter receives the binding associated with this key.
     *
