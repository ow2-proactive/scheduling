...
public class SmartProxyImpl extends AbstractSmartProxy<JobTrackerImpl> implements InitActive, EndActive, Serializable {
	...
	public boolean uploadInputfiles(TaskFlowJob job, String localInputFolderPath) throws Exception {
		...
		for (Task t : tasks) {
			...
			DataTransferProcessor dtp = new DataTransferProcessor(localfolder, remoteFolder, tfj.getName(), t.getName(),
					fileSelector);
	...
	public Page<TaskState> getTaskStates(String taskTag, long from, long to, boolean mytasks, boolean running,
			...
			throws NotConnectedException, PermissionException {
		return schedulerProxy.getTaskStates(taskTag, from, to, mytasks, running, pending, finished, offset, limit,
				sortParams);
	}
