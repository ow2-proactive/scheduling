...
public class RESTfulRMMonitoringUpdateClient extends BaseRESTfulRMClient {
    ...
    public Arguments getDefaultParameters() {
        ...
        defaultParameters.addArgument(PARAM_USE_DEFAULT_JMX_URL,
                "${useDefaultJmxUrl}");
    ...
    protected void doSetupTest(JavaSamplerContext context) throws Throwable {
        ...
        useDefaultJmxUrl = Boolean.parseBoolean(context
                .getParameter(PARAM_USE_DEFAULT_JMX_URL));
    }
    ...
    protected SampleResult doRunTest(JavaSamplerContext context)
        ...
        if (rmStateResult.isSuccessful()) {
            RmStateView rmState = getObjectMapper().readValue(
                    rmStateResult.getResponseData(), RmStateView.class);
    ...
    private void setNodeInfo(NodeEventView[] nodes) throws Exception {
        ...
        for (NodeEventView nodeView : nodes) {
            Node node = new Node(nodeView.getHostName(), nodeView.getNodeUrl(),
                    nodeJmxUrl(nodeView));
            nodeList.add(node);
            if (hostMap.get(nodeView.getHostName()) == null) {
                hostMap.put(nodeView.getHostName(),
                        new Host(nodeView.getHostName(), node.getJmxUrl()));
            }
        }
        JMeterUtils.setProperty(clientSpecificHostPropKey(), getObjectMapper()
                .writeValueAsString(hostMap.values()));
        JMeterUtils.setProperty(clientSpecificNodePropKey(), getObjectMapper()
                .writeValueAsString(nodeList));
    }

    private String nodeJmxUrl(NodeEventView nodeView) {
        return (useDefaultJmxUrl) ? nodeView.getDefaultJMXUrl() : nodeView
                .getProactiveJMXUrl();
    }
