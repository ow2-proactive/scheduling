...
public class RESTfulSchedJobClient extends BaseRESTfulSchedClient {
    ...
    public Arguments getDefaultParameters() {
        ...
        defaultParameters.addArgument(PARAM_JOB_STATUS_REFRESH_TIME,
    ...
    protected SampleResult doRunTest(JavaSamplerContext context)
        ...
        if (!isEmpty(availableJobIds)) {
            for (String jobId : availableJobIds) {
                ...
                JobStateView jobState = getObjectMapper().readValue(
                        jobStateResult.getResponseData(), JobStateView.class);
                for (TaskStateView taskState : jobState.getTasks().values()) {
                    result.addSubResult(fetchTaskOutput(jobId,
                            taskName(taskState)));
                    if (STATUS_FINISHED.equals(taskStatus(taskState))) {
                        result.addSubResult(fetchTaskResult(jobId,
                                taskName(taskState)));
                    }
                }
            }
        } else {
            logInfo(String.format("%s: Job ids not available", Thread
                    .currentThread().toString()));
            result.sampleEnd();
        }
        result.setSuccessful(true);
        waitForNextCycle(getTimestamp(),
                context.getIntParameter(PARAM_JOB_STATUS_REFRESH_TIME));
        return result;
    }
    ...
    private SampleResult fetchTaskResult(String jobId, String taskName) {
        String resourceUrl = (new StringBuilder(getConnection().getUrl()))
                .append("/jobs/").append(jobId).append("/tasks/")
                .append(encodePathElement(taskName)).append("/result")
                .toString();
    ...
    private SampleResult fetchJobState(String jobId) {
        String resourceUrl = (new StringBuilder(getConnection().getUrl()))
                .append("/jobs/").append(jobId).toString();
        return getResource(getClientSession(),
                String.format("job-state('%s')", jobId), resourceUrl);
    ...
    private SampleResult fetchTaskOutput(String jobId, String taskName) {
        String resourceUrl = (new StringBuilder(getConnection().getUrl()))
                .append("/jobs/").append(jobId).append("/tasks/")
                .append(encodePathElement(taskName)).append("/result/log/all")
                .toString();
        return getResource(getClientSession(),
                String.format("task-output(%s:%s)", jobId, taskName),
                resourceUrl);
    }
