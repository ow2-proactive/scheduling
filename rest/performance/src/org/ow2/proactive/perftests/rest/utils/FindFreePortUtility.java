...
public class FindFreePortUtility {
    ...
    public static Integer[] findFreePorts(InetAddress host, String hostJavaPath,
            String hostClassPath,  int requiredNumOfPorts)
        ...
        ProcessExecutor executor = SSHProcessExecutor.createExecutorSaveOutput(
                "findFreePorts", host, command);
        if (!executor.executeAndWaitCompletion(10000, true)) {
            throw new TestExecutionException(
                    "Failed to execute command to find free ports");
        }
        List<String> output = executor.getOutput();
        if (output.isEmpty()) {
            throw new TestExecutionException(
                    "Empty output for command finding free ports. Error output: "
