...
public class TestTomcatServerDeployer extends TestDeployer {
    ...
    private static final String PORTAL_PROP_TEMPLATE_FILE = PORTAL_CONFIG_DIR
            + separator + "portal.properties.template";

    private static final String TOMCAT_CONFIG_TEMPLATE_FILE = TOMCAT_CONFIG_DIR
            + separator + "server.xml.template";

    private static final String PA_CONFIG_FILE = "ProActiveConfiguration.xml";

    private static final String WEBAPP_DIR = "webapp";

    private static final String PORTAL_DIR = WEBAPP_DIR + separator
            + "SchedulingRest";

    private static final String PORTAL_PA_CONFIG_FILE = PORTAL_DIR + separator
            + "WEB-INF" + separator + PA_CONFIG_FILE;

    private static final String PORTAL_PROP_FILE = PORTAL_DIR + separator
            + "WEB-INF" + separator + "portal.properties";
    ...
    public static TestTomcatServerDeployer createAppServerDeployerUsingSystemProperties(
            String schedulerUrl, String rmUrl) throws Exception {
        String appServerHostName = TestUtils
                .getRequiredProperty("appserver.deploy.hostname");
        HostTestEnv serverHostEnv = new HostTestEnv(appServerHostName,
                TestEnv.getEnvUsingSystemProperties("appserver"));
        String protocol = TestUtils.getRequiredProperty("test.deploy.protocol");
        TestTomcatServerDeployer deployer = new TestTomcatServerDeployer(
                serverHostEnv, CLIENT_CONFIG_FILE, protocol, schedulerUrl,
                rmUrl);
        return deployer;
    }

    public TestTomcatServerDeployer(HostTestEnv serverHostEnv,
            String clientConfigFileName, String protocol, String schedulerUrl,
            String rmUrl) throws InterruptedException {
    ...
    public String getServerUrl() {
        return String.format("http://%s:%d/SchedulingRest/rest", serverHostEnv
                .getHost().getHostName(), httpPort);
    }
    ...
    protected String getPamrServedReservedId() {
        return TestUtils
                .getRequiredProperty("appserver.deploy.pamr.serverReservedId");
    }

    @Override
    public List<String> createServerStartCommand() {
        Config localConfig = new Config(getLocalSchedulingRestPath());
        Config hostConfig = new Config(getServerHostEnv().getHost(),
                getServerHostSchedulingRestPath());
        ...
        startup.add(asJvmParam("java.util.logging.config.file",
                absolutePath(tomcatDir, "conf/logging.properties")));
        startup.add(asJvmParam("java.util.logging.manager",
                "org.apache.juli.ClassLoaderLogManager"));
        String catalinaOpts = getCatalinaOpts();
        if (catalinaOpts != null) {
            startup.add(catalinaOpts);
        }
        startup.add(asJvmParam("java.endorsed.dirs",
                absolutePath(tomcatDir, "endorsed")));
        startup.add("-classpath");
        startup.add(absolutePath(tomcatDir, "bin/bootstrap.jar"));
        startup.add(asJvmParam("java.security.manager"));
        startup.add(asJvmParam("java.security.policy",
                absolutePath(testConfigDir, "grant.all.java.policy")));
        startup.add(asJvmParam("catalina.base", absolutePath(tomcatDir)));
        startup.add(asJvmParam("catalina.home", absolutePath(tomcatDir)));
        startup.add(asJvmParam("java.io.tmpdir",
                absolutePath(testTmpDir, "temp")));
    ...
    private void createPortalConfig(String schedulerUrl, String rmUrl,
            Config localConfig, Config hostConfig) {
        ...
        try {
            ...
            String portalProperties = TestFileUtils
                    .readStreamToString(new FileInputStream(new File(
                            testConfigDir, PORTAL_PROP_TEMPLATE_FILE)));
            portalProperties = portalProperties.replace("@SCHEDULER_URL@",
                    schedulerUrl).replace("@RM_URL@", rmUrl);
            TestFileUtils.writeStringToFile(localFile, portalProperties);
            copyFileFromLocalToHost(localFile, localConfig, hostConfig);

        } catch (Exception e) {
            throw new TestExecutionException(
                    "An error occurred while creating portal.properties file.",
                    e);
        }
    }
    ...
    private void createPAConfigFile(Config localConfig, Config hostConfig) {
        try {
            ...
            Map<String, String> clientProperties = getClientJavaProperties(getServerHostEnv()
                    .getEnv());
            String config = DeploymentTestUtils
                    .createProActiveConfiguration(clientProperties);
            
            TestFileUtils.writeStringToFile(localFile, config);
            copyFileFromLocalToHost(localFile, localConfig, hostConfig);
            
            File clientPAConfigFile = new File(localConfig.getTestTmpDir(),
                    PA_CONFIG_FILE);
            TestFileUtils.writeStringToFile(clientPAConfigFile, config);
        } catch (Exception e) {
            throw new TestExecutionException(
                    "An error occurred while creating ProActiveConfiguration file.",
                    e);
        }
    ...
    private void createTestServerConfig(Config localConfig, Config hostConfig) {
        try {
            File localWebappDir = new File(localConfig.getTestTmpDir(),
                    "webapp");
            String hostWebappDir = convertFileNameFromLocalToHost(
                    localWebappDir, localConfig, hostConfig);

            Integer[] freePorts = FindFreePortUtility.findFreePorts(
                    getServerHostEnv().getHost(), getServerHostEnv().getEnv()
                            .getJavaPath(), hostConfig.getTestClassesDir()
                            .getAbsolutePath(), 4);

            httpPort = freePorts[0].intValue();

            File localFile = new File(localConfig.getTestTmpDir(),
                    TOMCAT_CONFIG_FILE);
            String serverConfigTemplate = TestFileUtils
                    .readStreamToString(new FileInputStream(new File(
                            localConfig.getTestConfigDir(),
                            TOMCAT_CONFIG_TEMPLATE_FILE)));
            String serverConfig = serverConfigTemplate
                    .replace("@HTTP_PORT@", Integer.toString(httpPort))
                    .replace("@REDIRECT_PORT@", freePorts[1].toString())
                    .replace("@AJP_PORT@", freePorts[2].toString())
                    .replace("@SHUTDOWN_PORT@", freePorts[3].toString())
            ...
            throw new TestExecutionException(
                    "An error occurred while creating server.xml file.", e);
        }
    }

    private String convertFileNameFromLocalToHost(File localFile,
            Config localConfig, Config hostConfig) {
        String localFileName = localFile.getAbsolutePath();
        String targetFileName = localFileName.replace(localConfig.getRootDir()
                .getAbsolutePath(), hostConfig.getRootDir().getAbsolutePath());
        return targetFileName;
    }

    private void copyFileFromLocalToHost(File localFile, Config localConfig,
            Config hostConfig) throws InterruptedException {
        String hostFileName = convertFileNameFromLocalToHost(localFile,
                localConfig, hostConfig);
        createFileInHost(localFile.getAbsolutePath(), hostFileName,
                hostConfig.getHost());
    }

    private void createFileInHost(String localFileName, String hostFileName,
            InetAddress host) throws InterruptedException {
        List<String> command = new ArrayList<String>();
        command.add("rsync");
        command.add(localFileName);
        command.add(host.getHostName() + ":" + hostFileName);
        ProcessExecutor rsync = new ProcessExecutor("rsync", command, false,
                true);
        if (!rsync.executeAndWaitCompletion(10000, true)) {
            throw new TestExecutionException("Failed to copy file '"
                    + localFileName + "' to the " + host + ":" + hostFileName);
        }
    ...
    private void createHostWebappDirectory(Config hostConfig) {
        try {
            File testTmpDir = hostConfig.getTestTmpDir();
            String testPortalDir = (new File(testTmpDir, PORTAL_DIR))
                    .getAbsolutePath();
            String zipFile = getServerHostWarFile();
            ZipUtility.unzipFile(zipFile, testPortalDir, serverHostEnv);
        } catch (Exception e) {
            throw new TestExecutionException(
                    "Failed to create webapp directory", e);
        }
    }

    private String getCatalinaOpts() {
        String catalinaOpts = System.getProperty("test.deploy.env."
                + serverHostEnvName() + ".catalina_opts");
        return catalinaOpts;
    }

    private String getLocalSchedulingRestPath() {
        return TestUtils.getRequiredProperty("test.deploy.env." + "local"
                + ".schedulingRestPath");
    }

    private String getServerHostSchedulingRestPath() {
        return TestUtils.getRequiredProperty("test.deploy.env."
                + serverHostEnvName() + ".schedulingRestPath");
    }

    private String getServerHostTomcatDir() {
        return TestUtils.getRequiredProperty("test.deploy.env."
                + serverHostEnvName() + ".tomcathome");
    }

    private String getServerHostWarFile() {
        return TestUtils.getRequiredProperty("test.deploy.env."
                + serverHostEnvName() + ".warfile");
    }
    ...
    private String asJvmParam(String option, String value) {
        return (new StringBuilder()).append("-D").append(option).append('=')
    ...
    private class Config {
        ...
        public Config(InetAddress host, String rootDirPath) {
            ...
            testClassesDir = new File(rootDir, "classes" + File.separator
                    + "performance");
