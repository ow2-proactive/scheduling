...
public class TestServerDeployer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        File resultPropertiesFile = TestFileUtils.getWritableFile(deployResultPropertiesFileName);
        ...
        System.out
                .println(String
                        .format(
                                "RestSetupDeployer, startPamrIfNeeded: %s, startRM: %s, startScheduler: %s, startAppServer: %s",
                                startPamrIfNeeded, startRM, startScheduler, startAppServer));
        ...
        if ((protocol.equalsIgnoreCase("pamr") || protocol.equalsIgnoreCase("multi")) && startPamrIfNeeded) {
            try {
                ...
                System.setProperty(TestPamrProtocolHelper.PAMR_EXISTING_ROUTER_HOST, deployer.getPamrHost()
                        .getHostName());
                System.setProperty(TestPamrProtocolHelper.PAMR_EXISTING_ROUTER_PORT, String.valueOf(deployer
                        .getPamrPort()));
                System.out.println("PAMR router deployment finished succesfully.");
            } catch (Throwable t) {
        ...
        if (startRM) {
            ...
            try {
                TestRMDeployer testRMDeployer = TestRMDeployer.createRMDeployerUsingSystemProperties();
                Map<String, String> rmDeployProperties = deploy(testRMDeployer, "rm");
        ...
        if (startScheduler) {
            ...
            try {
                ...
                Map<String, String> schedulerDeployProperties = deploy(testSchedulerDeployer, "scheduler");
            ...
            if (deploymentResult) {
                System.out.println("Scheduler deployment finished successfully");
            } else {
        ...
        if (startAppServer) {
            ...
            try {
                TestTomcatServerDeployer testAppServerDeployer = TestTomcatServerDeployer
                        .createAppServerDeployerUsingSystemProperties(schedulerUrl, rmUrl);
                Map<String, String> appServerDeployProperties = deploy(testAppServerDeployer, "appserver");
                if (appServerDeployProperties != null) {
                    deploymentResult = true;
                    appServerDeployProperties.put("appserver.deploy.result.serverUrl", testAppServerDeployer
                            .getServerUrl());
            ...
            if (deploymentResult) {
                System.out.println("AppServer deployment finished successfully");
            } else {
