...
public abstract class BaseRESTfulClient extends BaseJMeterClient {
    ...
    protected SampleResult getResource(String session, String resourceType, String resourceUrl) {
        ...
        try {
            ...
            logError(String.format("%s An error occurred while retrieving %s: %nUrl: %s", Thread
                    .currentThread().toString(), resourceType, resourceUrl), ioe);
        } finally {
            result.sampleEnd();
        }
        if (response != null) {
            if (STATUS_OK == response.getStatusCode()) {
                ...
                logError(String.format("%s An error occurred while retrieving %s: %n%s", Thread
                        .currentThread().toString(), resourceType, new String(response.getContents())));
            }
        }
    ...
    protected void waitForNextCycle(long startTimeInMillis, int refreshPeriodInSecs) {
        if (startTimeInMillis == -1) {
            return;
        }
        long remainder = (refreshPeriodInSecs * 1000) - (System.currentTimeMillis() - startTimeInMillis);
        if (remainder > 0) {
            try {
    ...
    protected void doSetupTest(JavaSamplerContext context) throws Throwable {
        objectMapper = new ObjectMapper().configure(Feature.FAIL_ON_UNKNOWN_PROPERTIES, false);
    }
