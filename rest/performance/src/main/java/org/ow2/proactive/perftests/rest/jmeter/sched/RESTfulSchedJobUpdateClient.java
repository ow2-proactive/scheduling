...
public class RESTfulSchedJobUpdateClient extends BaseRESTfulSchedClient {
    ...
    public Arguments getDefaultParameters() {
        ...
        defaultParameters.addArgument(PARAM_JOB_CLIENT_REFRESH_TIME, "${jobClientRefreshTime}");
    ...
    protected SampleResult doRunTest(JavaSamplerContext context) throws Throwable {
        ...
        SampleResult result = getResource(getClientSession(), "job ids", resourceUrl);
        if (result.isSuccessful()) {
            @SuppressWarnings("unchecked")
            List<String> jobIds = (List<String>) getObjectMapper().readValue(result.getResponseData(),
                    new TypeReference<List<String>>() {
                    });
            if (jobIds != null && !jobIds.isEmpty()) {
                JMeterUtils.setProperty(clientSpecificJobIdKey(), toString(jobIds));
            }
        }
        waitForNextCycle(getTimestamp(), context.getIntParameter(PARAM_JOB_CLIENT_REFRESH_TIME));
        return result;
    }
