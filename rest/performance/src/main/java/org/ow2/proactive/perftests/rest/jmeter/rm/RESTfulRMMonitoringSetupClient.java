...
public class RESTfulRMMonitoringSetupClient extends BaseRESTfulRMClient {
    ...
    protected SampleResult doRunTest(JavaSamplerContext context) throws Throwable {
        ...
        HttpPost request = new HttpPost(getConnection().getUrl() + "/rm/login");
        StringEntity entity = new StringEntity((new StringBuilder()).append("username=").append(
                getConnection().getLogin()).append("&password=").append(getConnection().getPassword())
        ...
        try {
            ...
            logError(String.format("%s: An error occurred while login:", Thread.currentThread().toString()),
                    ioe);
        }
        ...
        if (response != null) {
            if (STATUS_OK == response.getStatusCode()) {
                ...
                logError(String.format("%s: An error occurred while login: %n%s", Thread.currentThread()
                        .toString(), new String(response.getContents())));
            }
        }
