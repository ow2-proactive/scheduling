...
public class MBeanQueries {
    ...
    // attributes
    private static final String[] ATTR_MONITORING = new String[] { "ManagementSpecVersion", "Name",
            "SpecName", "SpecVendor", "StartTime", "Uptime", "VmName", "VmVendor", "VmVersion",
            "BootClassPath", "ClassPath", "LibraryPath" };
    private static final String[] ATTR_HOST_OVERVIEW = new String[] { "Name", "Arch", "Version" };
    private static final String[] ATTR_CPU_USAGE = new String[] { "Combined" };
    private static final String[] ATTR_CPU = new String[] { "CacheSize", "CoresPerSocket", "Idle", "Irq",
            "Mhz", "Model", "Nice", "SoftIrq", "Sys", "Total", "TotalCores", "TotalSockets", "User",
            "Vendor", "Wait" };
    private static final String[] ATTR_MEM = new String[] { "ActualUsed", "ActualFree", "Total" };
    private static final String[] ATTR_SWAP = new String[] { "Used", "Free", "Total" };
    private static final String[] ATTR_FS = new String[] { "DevName", "DirName", "Files", "Options",
            "SysTypeName", "Free", "Used", "Total" };
    private static final String[] ATTR_FS_TOT = new String[] { "Total" };
    private static final String[] ATTR_NETWORK = new String[] { "DefaultGateway", "DomainName", "FQDN",
            "HostName", "PrimaryDns", "SecondaryDns" };
    ...
    // query
    public static final String QUERY_MONITORING = buildQuery(MBEAN_MONITORING, ATTR_MONITORING);
    public static final String QUERY_HOST_OVERVIEW = buildQuery(MBEAN_HOST_OVERVIEW, ATTR_HOST_OVERVIEW);
    public static final String QUERY_CPU_USAGE = buildQuery(MBEAN_CPU_USAGE, ATTR_CPU_USAGE);
    ...
    public static final String QUERY_NETWORK_INF = buildQuery(MBEAN_NETINF, ATTR_NETINF);
    public static final String QUERY_NETWORK = buildQuery(MBEAN_NETINF, ATTR_NETWORK);
    public static final String QUERY_HEAP_MEM = buildQuery(MBEAN_HEAP_MEM, ATTR_HEAP_MEM);
    public static final String QUERY_THREADS = buildQuery(MBEAN_THREADS, ATTR_THREADS);
    public static final String QUERY_CLASSES = buildQuery(MBEAN_CLASSES, ATTR_CLASSES);
    public static final String QUERY_JVM_CPU_USAGE = buildQuery(MBEAN_JVM_CPU_USAGE, ATTR_JVM_CPU_USAGE);
    public static final String QUERY_PROCESSES = buildQuery(MBEAN_PROCESSES, ATTR_PROCESSES);
    ...
    public static String getQueryUrl(String serverUrl, String resourcePath, String nodeJmxUrl,
            String attrQuery) {
        StringBuilder buffer = new StringBuilder();
        buffer.append(serverUrl).append(resourcePath).append('?');
        try {
            buffer.append(encode("nodejmxurl")).append('=').append(encode(nodeJmxUrl));
    ...
    private static String buildQuery(String mbean, String[] attributes) {
        StringBuilder buffer = new StringBuilder();
        try {
            buffer.append('&').append(encode("objectname")).append('=').append(encode(mbean));
            int index = 0;
            if (attributes != null) {
                while (index < attributes.length) {
                    buffer.append('&').append(encode("attr")).append('=').append(encode(attributes[index++]));
                }
            }
        } catch (UnsupportedEncodingException e) {
            throw new RuntimeException(e);
    ...
    private static String encode(String unescaped) throws UnsupportedEncodingException {
