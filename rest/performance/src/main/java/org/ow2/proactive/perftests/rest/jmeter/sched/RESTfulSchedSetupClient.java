...
public class RESTfulSchedSetupClient extends BaseRESTfulSchedClient {
    ...
    protected SampleResult doRunTest(JavaSamplerContext context) throws Throwable {
        ...
        String resourceUrl = (new StringBuffer(getConnection().getUrl())).append("/login").toString();
        HttpPost request = new HttpPost(resourceUrl);
        StringEntity entity = new StringEntity((new StringBuilder()).append("username=").append(
                getConnection().login()).append("&password=").append(getConnection().password()).toString(),
        ...
        try {
            ...
            throw new TestExecutionException(String.format(
                    "%s: An error occurred while retrieving the session id:", Thread.currentThread()
                            .toString()), ioe);
        } finally {
            result.sampleEnd();
        }
        if (response != null) {
            ...
            if (STATUS_OK == response.getStatusCode()) {
                ...
                throw new TestExecutionException(String.format(
                        "%s: An error occurred while retrieving session id: %n%s", Thread.currentThread()
