...
try {
    ...
    job.addTask(task)
    scheduler.connect()
    scheduler.pushFile("USERSPACE","/","inFile.txt", localspace + "/inFile.txt")
    jobid = scheduler.submit(job)
    println("job submitted");
    taskResult = scheduler.waitForTask(jobid.toString(), "HelloTask", 120000)
    scheduler.pullFile("USERSPACE", "/outFile.txt", localspace + "/outFile.txt")
