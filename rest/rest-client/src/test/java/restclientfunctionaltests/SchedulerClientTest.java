...
import restserverfunctionaltests.jobs.ErrorTask;
import restserverfunctionaltests.jobs.LogTask;
import restserverfunctionaltests.jobs.MetadataTask;
import restserverfunctionaltests.jobs.NonTerminatingJob;
import restserverfunctionaltests.jobs.SimpleJob;
import restserverfunctionaltests.jobs.VariableTask;
...
public class SchedulerClientTest extends AbstractRestFuncTestCase {
    ...
    public void testSchedulerNodeClient() throws Throwable {
        ISchedulerClient client = clientInstance();
        Job job = nodeClientJob("/restclientfunctionaltests/descriptors/scheduler_client_node.groovy",
                "/restclientfunctionaltests/descriptors/scheduler_client_node_fork.groovy");
        JobId jobId = submitJob(job, client);
    ...
    public void testDataSpaceNodeClientPushPull() throws Throwable {
        ISchedulerClient client = clientInstance();
        Job job = nodeClientJob("/restclientfunctionaltests/descriptors/dataspace_client_node_push_pull.groovy",
                "/restclientfunctionaltests/descriptors/dataspace_client_node_fork.groovy");
        JobId jobId = submitJob(job, client);
        TaskResult tres = client.waitForTask(jobId.toString(), "NodeClientTask", TimeUnit.MINUTES.toMillis(5));
        System.out.println(tres.getOutput().getAllLogs(false));
        Assert.assertNotNull(tres);
    ...
    public void testDataSpaceNodeClientPushDelete() throws Throwable {
        ISchedulerClient client = clientInstance();
        Job job = nodeClientJob("/restclientfunctionaltests/descriptors/dataspace_client_node_push_delete.groovy",
                "/restclientfunctionaltests/descriptors/dataspace_client_node_fork.groovy");
        JobId jobId = submitJob(job, client);
        TaskResult tres = client.waitForTask(jobId.toString(), "NodeClientTask", TimeUnit.MINUTES.toMillis(5));
        System.out.println(tres.getOutput().getAllLogs(false));
