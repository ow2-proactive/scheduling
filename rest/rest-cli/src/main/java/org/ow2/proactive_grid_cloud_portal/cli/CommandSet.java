...
public class CommandSet {
    ...
    public static final CommandSet.Entry URL =
            CommandSetEntryBuilder.newInstance().opt("u").longOpt("url")
                    .description("URL of REST server").hasArgs(true).numOfArgs(1).argNames("server-url")
                    .jsCommand("url(url)").commandClass(SetUrlCommand.class).entry();

    public static final CommandSet.Entry SESSION_ID =
            CommandSetEntryBuilder.newInstance().opt("si").longOpt("session-id")
                    .description("the session id of this session").hasArgs(true).numOfArgs(1)
                    .argNames("session-id").jsCommand("setSessionId(sessionId)")
    ...
    public static final CommandSet.Entry CREDENTIALS =
            CommandSetEntryBuilder.newInstance().opt("c").longOpt("credentials")
                    .description("Path to the credential file").hasArgs(true).numOfArgs(1)
                    .argNames("cred-path").jsCommand("loginwithcredentials(cred-path)")
                    .commandClass(LoginWithCredentialsCommand.class).entry();

    public static final CommandSet.Entry CA_CERTS =
            CommandSetEntryBuilder.newInstance().opt("ca").longOpt("cacerts")
                    .description("CA certificate store (JKS type) to verify peer against (SSL)")
                    .hasArgs(true).numOfArgs(1).argNames("store-path").jsCommand("cacerts(store-path)")
                    .commandClass(SetCaCertsCommand.class).entry();

    public static final CommandSet.Entry CA_CERTS_PASSWORD =
            CommandSetEntryBuilder.newInstance().opt("cap").longOpt("cacertspass")
                    .description("Password for CA certificate store").hasArgs(true).numOfArgs(1)
                    .argNames("store-password").jsCommand("cacertspass(cacerts-pass)")
                    .commandClass(SetCaCertsPassCommand.class).entry();

    public static final CommandSet.Entry INSECURE =
            CommandSetEntryBuilder.newInstance().opt("k").longOpt("insecure")
                    .description("Allow connections to SSL sites without certs verification")
                    .commandClass(SetInsecureAccessCommand.class).entry();

    public static final CommandSet.Entry SILENT =
            CommandSetEntryBuilder.newInstance().opt("z").longOpt("silent")
                    .description("Runs the command-line client in the silent mode.")
                    .jsCommand("silent()").commandClass(SetSilentCommand.class).entry();

    public static final CommandSet.Entry PRINT =
            CommandSetEntryBuilder.newInstance().opt("pr").longOpt("print")
    ...
    public static final CommandSet.Entry DEBUG =
            CommandSetEntryBuilder.newInstance().opt("X").longOpt("debug")
    ...
    public static final CommandSet.Entry SCHEDULER_START =
            CommandSetEntryBuilder.newInstance().opt("start").longOpt("startscheduler")
                    .description("Start the Scheduler").jsCommand("start()")
                    .commandClass(StartCommand.class).entry();

    public static final CommandSet.Entry SCHEDULER_STOP =
            CommandSetEntryBuilder.newInstance().opt("stop").longOpt("stopscheduler")
                    .description("Stop the Scheduler").jsCommand("stop()")
                    .commandClass(StopCommand.class).entry();

    public static final CommandSet.Entry SCHEDULER_PAUSE =
            CommandSetEntryBuilder.newInstance().opt("pause").longOpt("pausescheduler")
                    .description("Pause the Scheduler (pause all non-running jobs)")
                    .jsCommand("pause()").commandClass(PauseCommand.class).entry();

    public static final CommandSet.Entry SCHEDULER_RESUME =
            CommandSetEntryBuilder.newInstance().opt("resume").longOpt("resumescheduler")
                    .description("Resume the Scheduler").jsCommand("resume()")
                    .commandClass(ResumeCommand.class).entry();

    public static final CommandSet.Entry SCHEDULER_FREEZE =
            CommandSetEntryBuilder.newInstance().opt("freeze").longOpt("freezescheduler")
                    .description("Freeze the Scheduler (pause all non-running jobs)")
                    .jsCommand("freeze()").commandClass(FreezeCommand.class).entry();

    public static final CommandSet.Entry SCHEDULER_KILL =
            CommandSetEntryBuilder.newInstance().opt("kill").longOpt("killscheduler")
    ...
    public static final CommandSet.Entry JOB_RESTART_ALL_IN_ERROR_TASKS =
            CommandSetEntryBuilder.newInstance().opt("raiet").longOpt("restart-all-in-error-tasks")
                    .description("Restart all In-Error tasks for the specified job")
                    .hasArgs(true).numOfArgs(1).argNames("job-id")
                    .jsCommand("restartAllInErrorTasks(job_id)")
                    .commandClass(RestartAllInErrorTasksCommand.class).entry();
    ...
    public static final CommandSet.Entry NODE_REMOVE =
            CommandSetEntryBuilder.newInstance().opt("d").longOpt("removenodes")
                    .description("Remove the specified node").hasArgs(true).numOfArgs(1)
                    .argNames("node-url").jsCommand("removenode(node-url)")
                    .commandClass(RemoveNodeCommand.class).entry();

    public static final CommandSet.Entry FORCE =
            CommandSetEntryBuilder.newInstance().opt("f").longOpt("force")
     ...
     * Manager CLIs
     */
    public static final CommandSet.Entry[] COMMON_COMMANDS =
            new CommandSet.Entry[] {
                    URL, SESSION_ID, SESSION_ID_FILE, PASSWORD, CREDENTIALS, INSECURE, CA_CERTS,
                    CA_CERTS_PASSWORD, EVAL, SILENT, PRINT, OUTPUT, COMMON_HELP, DEBUG, VERSION };

    /**
     * CommandSet.Entry objects which are specific to Scheduler CLI
     */
    public static final CommandSet.Entry[] SCHEDULER_ONLY =
            new CommandSet.Entry[] {
