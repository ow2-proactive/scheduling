...
public class UserGuide {
	...
	public static NodeSet getNodes(ResourceManager resourceManager, int nbOfNodes, SelectionScript selectionScript){
	...
	private static SelectionScript getLinuxScript() {
		...
		String script = "" +
			"/* Check if OS name is Linux and their is wireless connection available */" +
	...
	private static SelectionScript getWindowsScript() {
		...
		SelectionScript selectionScript = null;
		try {
			selectionScript = new SelectionScript( new File(PAResourceManagerProperties.RM_HOME + "samples/scripts/selection/checkWindows.js"), new String[] {}, false);
		} catch (InvalidScriptException e) {
			e.printStackTrace();
		}
