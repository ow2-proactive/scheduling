...
public class ActiveObjectClass implements java.io.Serializable {	
    ...
    public String checkIfTotalIsStarted(){
    	CoreTimersContainer c =  (CoreTimersContainer)TimerWarehouse.getTimerProvidable(ProActive.getBodyOnThis().getID());
    	// Check if the timer container is null
    	if ( c == null ) return "There is no TimerProvidable registered for this body id.";
    ...
    public String checkIfWfrIsStopped(){
    	...
    	if ( wfrTimer.getTotalTime() <= 0 ){
    		return "Problem with the WaitForRequest timer its time is : " + wfrTimer.getTotalTime();
    	}
    	return "true";
    }
    ...
    public String performSyncCallOnRemote(){
    	...
    	if ( timedSerTime <= 0 ||
    			timedSerTime > realSendRequestTime	||
