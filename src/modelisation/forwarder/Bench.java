...
public class Bench implements modelisation.forwarder.ActiveWithForwarder {


    protected static NodeControler auto;

    public static AgentWithExponentialMigrationAndForwarder startExponentialAgent(double d,
                                                                                  Node[] nodes,
                                                                                  String nodeName,
                                                                                  long lifeTime) {
        ...
        try {
            agent = (AgentWithExponentialMigrationAndForwarder)
                    ProActive.newActive("modelisation.forwarder.AgentWithExponentialMigrationAndForwarder",
                                        args, NodeFactory.getNode(nodeName));
        } catch (Exception e) {
            e.printStackTrace();
        }
        //	agent.initialize(s);
    ...
    public Bench(AgentWithExponentialMigrationAndForwarder a,
                 ExponentialLaw e) {
        this.agent = a;
        this.expo = e;
    }
    ...
    public static void main(String args[]) {
        ...
        AgentWithExponentialMigrationAndForwarder agent =
                Bench.startExponentialAgent(Double.parseDouble(args[1]), nodes, args[3], Long.parseLong(args[4]));
        try {
            Thread.sleep(2000);
        } catch (Exception e) {
            e.printStackTrace();
