...
public class Bench implements modelisation.server.TimedActive {
    ...
    public static AgentWithExponentialMigrationAndServer startExponentialAgent(double d, Node[] nodes, String nodeName, long lifeTime) {
        ...
        try {
            agent = (AgentWithExponentialMigrationAndServer) ProActive.newActive(AgentWithExponentialMigrationAndServer.class.getName(), args, NodeFactory.getNode(nodeName));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return agent;
    ...
    public Bench(AgentWithExponentialMigrationAndServer a,
                 ExponentialLaw e, Long time) {
        this.agent = a;
        this.expo = e;
