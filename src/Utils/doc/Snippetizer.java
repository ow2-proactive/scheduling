		...
		for (int i = 0; i < elements.length; ++i) {
			...
			if (file.isDirectory()) {
				...
				//TODO remove constant
					...
					&& !file.getName().equals("Snippetizer.java")) {
					try {
						if (fileIsValid(file)) extractSnippets(file);
					} catch (Exception e) {
		...
		while ((line = reader.readLine()) != null) {
			if (line.contains(endAnnotation)) {
				...
				if (endTags.containsKey(endA)) {
					//TODO replace with log4j
					System.out.println("["+ lineCounter+ 
							"] WARNING: Duplicate stop tags ["+ endA +"] at " + 
			...
			if (line.contains(startAnnotation)) {
				...
				if (startTags.containsKey(startA)) {
					//TODO replace with log4j
					System.out.println("["+ lineCounter+ 
							"] WARNING: Duplicate start tags ["+ startA +"] at " + 
							"["+ lineCounter+ "] and [" + startTags.get(startA)  + "] " 
							+ ". File [" + file + "] will not be parsed and some code parts may" +
							" not appear in the final document");
					allOK=false;	
				}
...
//		if (endTags.size() > 0 ) System.out.println(endTags.keySet().toString());
		...
		for (String tag:startTags.keySet()){
			...
			if (endTags.get(tag) == null) {
				//TODO replace with log4j
				System.out.println("[" + startTags.get(tag) + 
						"] WARNING: Orphaned start tag ["+ tag +"] found at line:" + 
						"["+ startTags.get(tag)+ "]. File [" + file + "] will not be parsed and some code parts may" +
						"not appear in the final document." );
				allOK = false;
			}else
			{
				...
				if (endTags.get(tag)!=null) endTags.remove(tag);
			}
		}
		...
		if (endTags.size() != 0)
			for (String tag:endTags.keySet()){
				//TODO replace with log4j
				System.out.println("[" + endTags.get(tag) + 
						"] WARNING: Orphaned end tag ["+ tag +"] found at line:" + 
						"["+ endTags.get(tag)+ "]. File [" + file + "] will not be parsed and some code parts may" +
						"not appear in the final document." );
				allOK = false;
			}
	...
	public BufferedWriter createFile(String file) throws Exception {
		...
		if (targetFile.exists()) {
			...
			System.out.println("WARNING: File " + targetFile + " already exists and it will be overwritten. " +
	...
	public static void main(String[] args) throws Exception {
		...
		Snippetizer  parser = new Snippetizer(new File(sourceDir.getAbsolutePath()),
				targetDir);
