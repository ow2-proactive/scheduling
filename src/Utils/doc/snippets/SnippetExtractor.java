...
public abstract class SnippetExtractor implements Runnable {
	...
	protected static Logger logger = Logger
			.getLogger(SnippetExtractor.class.getName());
	 ...
	 */
	public SnippetExtractor(final File file, final File targetDir,
			final String startA, final String endA, final String startE,
			final String endE) {
	 ...
	 */
	public SnippetExtractor(final File file, final File targetDir,
			final String startA, final String endA) {
	...
	public void run() {
		try {
			this.reader = new BufferedReader(new InputStreamReader(
					new FileInputStream(this.target)));
			// check if the file is valid and then parse
			if (this.fileIsValid()){
				SnippetExtractor.logger.debug("File is valid, trying to extract: "
						+ this.target);
				this.reader = new BufferedReader(new InputStreamReader(
						new FileInputStream(this.target)));
			...
			SnippetExtractor.logger.error("Extraction error for file: "
					+ this.target + " " + ioExcep.getMessage());
		}
	}
	...
	private boolean fileIsValid() throws IOException {
		...
		while ((line != null) && fileValid) {
			if (line.contains(this.endAnnotation)) {
				...
				SnippetExtractor.logger.debug("Found end tag [" 
						+ endA + "]" + "at line " + lineCounter);
				// check if the tag is not empty
				if (endA.length() == 0) {
					SnippetExtractor.logger.error("[" + lineCounter
							+ "]  Empty tag found at " + "[" + lineCounter
							+ "]. File [" + this.target
							+ "] will not be parsed and some code parts may"
				...
				if (endTags.containsKey(endA)) {
					SnippetExtractor.logger.error("[" + lineCounter
							+ "]  Duplicate stop tags [" + endA + "] at " + "["
							+ lineCounter + "] and [" + endTags.get(endA)
							+ "] " + ". File [" + this.target
							+ "] will not be parsed and some code parts may"
							+ " not appear in the final document");
					fileValid = false;
			...
			if (line.contains(this.startAnnotation)) {
				...
				SnippetExtractor.logger.debug("Found start tag [" 
						+ startA + "]" + "at line " + lineCounter);
				if (startA.length() == 0) {
					SnippetExtractor.logger.error("[" + lineCounter
							+ "]  Empty tag found at " + "[" + lineCounter
							+ "]. File [" + this.target
				...
				if (startTags.containsKey(startA)) {
					SnippetExtractor.logger.error("[" + lineCounter
							+ "]  Duplicate start tags [" + startA + "] at "
							+ "[" + lineCounter + "] and ["
							+ startTags.get(startA) + "] " + ". File ["
							+ this.target
							+ "] will not be parsed and some code parts may"
							+ " not appear in the final document");
					fileValid = false;
		...
		if (startTags.size() > 0)
			SnippetExtractor.logger.debug("Start tags extracted :"
					+ startTags.keySet().toString());
		if (endTags.size() > 0)
			SnippetExtractor.logger.debug("End tags extracted :"
					+ endTags.keySet().toString());
		...
		for (final Map.Entry<String, Integer> tag : startTags.entrySet()) {
			...
			// check for existence
			if (endValue == null) {
				SnippetExtractor.logger.error("[" + startValue
						+ "]  Orphaned start tag [" + startKey
						+ "] found at line:" + "[" + startValue + "]. File ["
						+ this.target
						+ "] will not be parsed and some code parts may"
						+ "not appear in the final document.");
				fileValid = false;
				...
				if (endValue <= startValue) {
					SnippetExtractor.logger.error("[" + endValue + ","
							+ startValue + "]  End tag [" + endKey
							+ "] found before start tag. End tag is at line:"
							+ "[" + endValue + "] and start tag is at line ["
							+ startValue + "] " + ". File [" + this.target
							+ "] will not be parsed and some code parts may"
							+ " not appear in the final document");
					fileValid = false;
				}
		...
		if (endTags.size() != 0)
			for (final Map.Entry<String, Integer> tag : endTags.entrySet()) {
				...
				SnippetExtractor.logger.error("[" + endValue
						+ "]  Orphaned end tag [" + endKey + "] found at line:"
						+ "[" + endValue + "]. File [" + this.target
						+ "] will not be parsed and some code parts may"
						+ "not appear in the final document.");
				fileValid = false;
			}
	...
	private void extractSnippets() throws IOException {
		...
		while (line != null) {
			...
			if ((writers.size() > 0) && (line.contains(this.endAnnotation))) {
				...
				SnippetExtractor.logger.debug("Removing --- " + endA
						+ "  line --- " + line);
				assert endA != null;
				writer.flush();
				writer.close();
				writers.remove(endA);
				SnippetExtractor.logger.debug(
						"---- Writers left after removal: "	+ writers);
			...
			if ((writers.size() > 0) && (!line.contains(this.endAnnotation))
				...
				for (final BufferedWriter buffer : writers.values()) {
					if (!line.contains(this.startAnnotation)
							&& !line.contains(this.endAnnotation)) {
						buffer.append(line);
					}
					buffer.newLine();
					...
					Integer value;
					for (final Map.Entry<String, Integer> currentWhiteSpace
							: whiteSpaceToRemove.entrySet()) {
						value = Math.min(line.length() - line.trim().length(),
								currentWhiteSpace.getValue());
						whiteSpaceToRemove.put(currentWhiteSpace.getKey(),
								value);
					}
				}
			...
			if (line.contains(this.startAnnotation)) {
				...
				if (targetFile.exists()) {
					SnippetExtractor.logger.warn(" File "
									+ targetFile + " already exists and it will NOT be overwritten. "
									+ " Either the directory has not been emptied" 
									+ " or there are global duplicate tags. The file(tag) name is"
									+ ":" + startA + ". The tag has be read from file "
									+ this.target);
				} else {
					SnippetExtractor.logger.debug("Adding ----" + startA
							+ " line --- " + line);
					assert startA != null;
					writers.put(startA, this.createFile(startA));
					SnippetExtractor.logger.info("File ["+ startA + "] created.");
					SnippetExtractor.logger.debug("++++ Writers after adding:"
							+ writers);
				}
	...
	private BufferedWriter createFile(final String file) {
		...
		try {
			...
			e.printStackTrace();
			SnippetExtractor.logger.error("File " + targetFile
	...
	private void formatFile(final String file, final int blanksToRemove) {
		...
		try {
			final BufferedReader fileReader = new BufferedReader(
					new InputStreamReader(new FileInputStream(parsedFile)));
			String line = null;
			final File outFile = new File(this.targetDirectory, file
					+ SnippetExtractor.FILE_EXTENSION);

			final BufferedWriter writer = new BufferedWriter(new FileWriter(
					outFile));
			String whiteSpaceToAdd;
			SnippetExtractor.logger.debug("Input file :" + parsedFile
					+ " output file " + outFile + " to remove "
					+ blanksToRemove);
