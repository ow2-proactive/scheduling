...
public class TestWorkflowIfJobs extends TWorkflowJobs {
    ...
    public final String[][] getJobs() {
        return new String[][] {
        // 1: if/else: single task; no join
                { "T 0", "T1 1", "T2 -1" },

                // 2 : if/else: task block; join
                { "T 0", "T1 1", "T3 2", "T5 3", "T2 -1", "T4 -1" },

                // 3 : if: task block w/ nested if/else single task; no join
                { "T 0", "T1 1", "T2 -1", "T3 -1", "T4 2", "T5 -1", "T6 3", "T7 4" },

                // 4 : if: task block; else: task w/ nested if: task block; join
                { "T 0", "T1 -1", "T2 -1", "T3 1", "T4 2", "T5 3", "T6 4", "T7 -1", "T8 5", "T9 6" },

                // 5 : nested if depth 3 with join
