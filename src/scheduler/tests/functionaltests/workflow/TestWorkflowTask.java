...
public class TestWorkflowTask {
	...
	public void run() throws Throwable {
		...
		JobId job = userInt.submit(JobFactory.getFactory().createJob(job_path));
		 ...
		 */
		Assert.assertTrue("Expected 8 task results", result.getAllResults()
				.size() == 8);
		for (String name : new String[] { "split", "split#1", "replica",
				"replica*1", "replica#1", "replica#1*1", "loop", "loop#1" }) {
			Assert.assertTrue("No result for task " + name, result
					.getAllResults().containsKey(name));
		}
		...
		for (TaskState t : userInt.getJobState(job).getTasks()) {
		 ...
		 */
		for (Entry<String, TaskResult> results : result.getAllResults()
				.entrySet()) {
			...
			if (name.startsWith("replica")) {
				 ...
				 * check task name 
				 */
				String resultRegex = ".*result " + tid.getIterationIndex()
						+ " " + tid.getReplicationIndex() + ".*";
				Assert.assertTrue("Unexpected result for " + name + ": "
						+ res.value().toString(), res.value().toString()
						.matches(resultRegex));
				 ...
				 */
				Assert.assertEquals(20, ts.getMaxNumberOfExecution());
				...
				for (InputSelector in : ts.getInputFilesList()) {
					...
					Assert.assertEquals(1,
							in.getInputFiles().getIncludes().length);

					String inc = in.getInputFiles().getIncludes()[0];
					Assert.assertTrue(inc.equals("foo_$IT_$REP.in")
							|| inc.equals("bar_$IT_$REP.in"));
				}
				/*
				 * check output files
				 */
				for (OutputSelector out : ts.getOutputFilesList()) {
					Assert.assertEquals(OutputAccessMode.TransferToOutputSpace,
							out.getMode());
					Assert.assertEquals(1,
							out.getOutputFiles().getIncludes().length);

					String inc = out.getOutputFiles().getIncludes()[0];
					Assert.assertTrue(inc.equals("foo_$IT_$REP.out")
							|| inc.equals("bar_$IT_$REP.out"));
				}

				/*
				 * Generic informations
				 */
				Assert.assertEquals(ts.getGenericInformations().get("info1"),
						"value1");
				Assert.assertEquals(ts.getGenericInformations().get("info2"),
						"value2");
				...
				if (tid.getIterationIndex() == 0) {
					Assert.assertEquals("split", ts.getDependences().get(0)
							.getName());
				} else {
					Assert.assertEquals("split#1", ts.getDependences().get(0)
							.getName());
				}
				 ...
				 * Topology
				 */
				Assert.assertTrue(SingleHostDescriptor.class
						.isAssignableFrom(ts.getParallelEnvironment()
				 ...
				 */
				Assert.assertTrue(out.contains("Pre script "
						+ tid.getIterationIndex() + " "
						+ tid.getReplicationIndex()));
				/*
				 * Post script execution
				 */
				Assert.assertTrue(out.contains("Post script "
						+ tid.getIterationIndex() + " "
						+ tid.getReplicationIndex()));
				/*
				 ...
				 */
				Assert.assertTrue(out.contains("generated hello "
						+ tid.getIterationIndex()));
