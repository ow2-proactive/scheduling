...
public class PAMapReduceJobMainJavaExecutable extends JavaExecutable {
	...
	public Serializable execute(TaskResult... results) throws Throwable {
		...
		try {
			 ...
			 */
			try {
				job.setNumReduceTasks(Integer
						.parseInt( numberOfReducers ));
		...
		System.out.println("The scheduler home is: " + pamrjc.getPropertyAsString(PAMapReduceFrameworkProperties.SCHEDULER_HOME.getKey()));
		System.out.println("The addons directory is: " + PAMapReduceFrameworkProperties.getPropertyAsString(PAMapReduceFrameworkProperties.SCHEDULER_ADDITIONAL_CLASSPATH_FOLDER.getKey()));
		...
		if (pamrj != null) {
			if (pamrj.run()) {
				System.out
						.println("The ProActive MapReduce job is correctly submitted!");
			} else {
				System.out
						.println("The ProActive MapReduce job is NOT submitted");
			}
		}
		...
		try {
			sai = SchedulerConnection
					.join(pamrjc.getPropertyAsString(PAMapReduceFrameworkProperties.SCHEDULER_URL.getKey()));
		...
		try {
			...
			System.out.println("The authentication to the scheduler is done using the credentials: " + System.getProperty(Credentials.credentialsPathProperty));
			...
			try {
				...
				System.out.println("The authentication to the scheduler cannot be done using the credentials: " + System.getProperty(Credentials.credentialsPathProperty));
				...
				try {
					String username = pamrjc.getPropertyAsString(PAMapReduceFrameworkProperties.SCHEDULER_USERNAME.getKey());
					String password = pamrjc.getPropertyAsString(PAMapReduceFrameworkProperties.SCHEDULER_PASSWORD.getKey());
