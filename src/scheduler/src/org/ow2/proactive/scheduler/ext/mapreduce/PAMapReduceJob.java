...
public class PAMapReduceJob {
	...
	public PAMapReduceJob(Job job, PAMapReduceJobConfiguration pamrjc)
		...
		checkConfiguration(paMapReduceJobConfiguration,
				initRequiredConfigurationProperties());
		init();
	}
	...
	protected void init() throws PAJobConfigurationException {
		if (!isInitialized()) {
			...
			if ((hadoopJob != null) && (paMapReduceJobConfiguration != null)) {
				...
				logger.debug("The ProActive Scheduler home is '"
						+ schedulerHomeString + "'");
				...
				if ((fileNameList != null) && (fileNameList.length > 0)) {
					for (String fileName : fileNameList) {
						forkEnvironment
								.addAdditionalClasspath(schedulerAdditionalClasspathFolder
										+ fileName);
					}
				}
				String[] jvmArgumentArray = paMapReduceJobConfiguration
						.getJVMArguments();
				if (jvmArgumentArray != null) {
					for (int i = 0; i < jvmArgumentArray.length; i++) {
						forkEnvironment.addJVMArgument(jvmArgumentArray[i]);
						logger.debug("Setting JVM argument '"
								+ jvmArgumentArray[i] + "'");
					}
				}
				...
				Configuration hadoopJobConfiguration = hadoopJob
						.getConfiguration();
				...
				paHadoopJobConfiguration
						.set(PAMapReduceFrameworkProperties
				...
				logger.debug("The Hadoop Abstract File System implementation is '"
						+ paHadoopJobConfiguration.get(PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_FS_IMPLEMENTATION_PROPERTY_NAME.key))
						+ "'");
				...
				logger.debug("The Hadoop Abstract File System implementation default name is '"
						+ paHadoopJobConfiguration.get(PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_FS_DEFAULT_NAME_PROPERTY_NAME.key))
						+ "'");
				...
				logger.debug("The Hadoop Abstract File System implementation enabled cache is '"
						+ paHadoopJobConfiguration.get(PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_FS_DISABLE_CACHE_PROPERTY_NAME.key))
						+ "'");
				...
				logger.debug("The input files of the Hadoop MapREduce job are '"
						+ paHadoopJobConfiguration.get(PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_INPUT_DIRECTORY_PROPERTY_NAME.key))
						+ "'");
				...
				tmpMapReduceWorkflow
						.setCancelJobOnError(paMapReduceJobConfiguration
								.getJobCancelOnError());
				logger.debug("The value of the cancelJobOnError attribute of the ProActive MapReduce job is '"
						+ paMapReduceJobConfiguration.getJobCancelOnError()
						+ "'");

				tmpMapReduceWorkflow
						.setRestartTaskOnError(paMapReduceJobConfiguration
				...
				tmpMapReduceWorkflow
						.setMaxNumberOfExecution(paMapReduceJobConfiguration
				...
				logger.debug("The value of the cancelJobOnError of the ProActive MapReduce SplitterPATask is '"
						+ paMapReduceJobConfiguration
								.getCancelJobOnError(PAMapReduceFramework.SPLITTER_PA_TASK)
						+ "'");
				...
				logger.debug("The value of the maxNumberOfExecutions of the ProActive MapReduce SplitterPATask is '"
						+ paMapReduceJobConfiguration
								.getMaxNumberOfExecutions() + "'");
				...
				logger.debug("The value of the precious logs of the ProActive MapReduce SplitterPATask is '"
						+ PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.WORKFLOW_JAVA_TASK_LOGGING_DEBUG.key)
						+ "'");
				 ...
				 */
				if (paMapReduceJobConfiguration.getReadMode(
						PAMapReduceFramework.SPLITTER_PA_TASK).equals(
						ReadMode.fullLocalRead)) {
					for (int i = 0; i < inputPathStringList.length; i++) {
						splitterPATask
								.addInputFiles(
										inputPathStringList[i],
										paMapReduceJobConfiguration
				...
				logger.debug("The value of the readMode of the ProActive MapReduce SplitterPATask is '"
						+ paMapReduceJobConfiguration
								.getReadMode(PAMapReduceFramework.SPLITTER_PA_TASK)
						+ "'");
				logger.debug("The value of the inputAccessMode of the ProActive MapReduce SplitterPATask is '"
						+ paMapReduceJobConfiguration
								.getInputAccessMode(PAMapReduceFramework.SPLITTER_PA_TASK)
						+ "'");
				...
				logger.debug("The value of the executablClassName of the ProActive MapReduce SplitterPATask is '"
						+ PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.WORKFLOW_SPLITTER_PA_TASK_EXECUTABLE_CLASS.key)
						+ "'");
				...
				splitterPATask
						.addArgument(
								...
								paMapReduceJobConfiguration
										.getInputSpace(PAMapReduceFramework.SPLITTER_PA_TASK));
				...
				splitterPATask
						.addArgument(
								PAMapReduceFrameworkProperties.HADOOP_JOB_CONFIGURATION.key,
								paHadoopJobConfiguration);

				InputStream replicateMapperPATaskInputStream = Thread
						.currentThread()
						.getContextClassLoader()
						.getResourceAsStream(
								PAMapReduceFramework.REPLICATE_MAPPER_PA_TASK_SCRIPT_NAME);
				...
				try {
					replicateMapperScript = new SimpleScript(
							...
							new String[] {
									SchedulerVars.JAVAENV_TASK_ID_VARNAME
											.toString(),
									PAMapReduceFrameworkProperties
											.getPropertyAsString(PAMapReduceFrameworkProperties.WORKFLOW_JAVA_TASK_LOGGING_DEBUG.key) });
				...
				try {
					replicateMapperFlowScript = FlowScript
							.createReplicateFlowScript(replicateMapperScript);
				} catch (InvalidScriptException ise) {
					ise.printStackTrace();
				}
				...
				logger.debug("The value of the cancelJobOnError of the ProActive MapReduce MapperPATask is '"
						+ paMapReduceJobConfiguration
								.getCancelJobOnError(PAMapReduceFramework.MAPPER_PA_TASK)
						+ "'");
				...
				logger.debug("The value of the maxNumberOfExecutions of the ProActive MapReduce MapperPATask is '"
						+ paMapReduceJobConfiguration
								.getMaxNumberOfExecutions() + "'");
				...
				mapperPATask
						.setPreciousLogs(PAMapReduceFrameworkProperties
								.getPropertyAsBoolean(PAMapReduceFrameworkProperties.WORKFLOW_JAVA_TASK_LOGGING_DEBUG.key));
				logger.debug("The value of the precious logs of the ProActive MapReduce MapperPATask is '"
						+ PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.WORKFLOW_JAVA_TASK_LOGGING_DEBUG.key)
						+ "'");
				...
				mapperPATask
						.addArgument(
								...
								paMapReduceJobConfiguration
										.getReadMode(PAMapReduceFramework.MAPPER_PA_TASK).key);
				logger.debug("The value of the readMode of the ProActive MapReduce MapperPATask is '"
						+ paMapReduceJobConfiguration
								.getReadMode(PAMapReduceFramework.MAPPER_PA_TASK)
						+ "'");
				...
				mapperPATask
						.addArgument(
								...
								paMapReduceJobConfiguration
										.getInputSpace(PAMapReduceFramework.MAPPER_PA_TASK));
				 ...
				 */
				if (paMapReduceJobConfiguration.getReadMode(
						PAMapReduceFramework.MAPPER_PA_TASK).equals(
						ReadMode.fullLocalRead)) {
					for (int i = 0; i < inputPathStringList.length; i++) {
						mapperPATask
								.addInputFiles(
				...
				logger.debug("The value of the executableClassName of the ProActive MapReduce MapperPATask is '"
						+ PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.WORKFLOW_MAPPER_PA_TASK_EXECUTABLE_CLASS.key)
						+ "'");
				...
				mapperPATask
						.addArgument(
								...
								paMapReduceJobConfiguration
										.getOutputSpace(PAMapReduceFramework.MAPPER_PA_TASK));
				 ...
				 * ReducerPATask)
				 */
				if (!(paMapReduceJobConfiguration
						.getWriteMode(PAMapReduceFramework.MAPPER_PA_TASK)
					...
					logger.debug("The value of the intermediateFileSelector of the ProActive MapReduce MapperPATask is '"
							+ outputFileName + "'");
					...
					logger.debug("The value of the outputAccessMode of the ProActive MapReduce MapperPATask is '"
							+ paMapReduceJobConfiguration
									.getOutputAccessMode(PAMapReduceFramework.MAPPER_PA_TASK)
							+ "'");
					...
					logger.debug("The value of the intermediateIndexFileSelector of the ProActive MapReduce MapperPATask is '"
							+ outputFileName + "'");
				...
				mapperJoinPATask
						.setDescription(paMapReduceJobConfiguration
				...
				mapperJoinPATask
						.setPreciousLogs(PAMapReduceFrameworkProperties
								.getPropertyAsBoolean(PAMapReduceFrameworkProperties.WORKFLOW_JAVA_TASK_LOGGING_DEBUG.key));
				logger.debug("The value of the precious logs of the ProActive MapReduce MapperJoinPATask is '"
						+ PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.WORKFLOW_JAVA_TASK_LOGGING_DEBUG.key)
						+ "'");
				...
				logger.debug("The value of the cancelJobOnError of the ProActive MapReduce MapperJoinPATask is '"
						+ paMapReduceJobConfiguration
								.getCancelJobOnError(PAMapReduceFramework.MAPPER_JOIN_PA_TASK)
						+ "'");
				...
				logger.debug("The value of the maxNumberOfExecutions of the ProActive MapReduce MapperJoinPATask is '"
						+ paMapReduceJobConfiguration
								.getMaxNumberOfExecutions(PAMapReduceFramework.MAPPER_JOIN_PA_TASK)
						+ "'");
				...
				logger.debug("The value of the executableClassName of the ProActive MapReduce MapperJoinPATask is '"
						+ PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.WORKFLOW_MAPPER_JOIN_PA_TASK_EXECUTABLE_CLASS.key)
						+ "'");
				...
				InputStream replicateReducerPATaskInputStream = Thread
						.currentThread()
						.getContextClassLoader()
						.getResourceAsStream(
					...
					replicateReducerScript = new SimpleScript(
							...
							new String[] {
									...
									PAMapReduceFrameworkProperties
											.getPropertyAsString(PAMapReduceFrameworkProperties.WORKFLOW_JAVA_TASK_LOGGING_DEBUG.key),
									""
											+ paHadoopJobConfiguration.getInt(
													PAMapReduceFrameworkProperties
				...
				try {
					replicateReducerFlowScript = FlowScript
							.createReplicateFlowScript(replicateReducerScript);
				} catch (InvalidScriptException ise) {
					ise.printStackTrace();
				}
				...
				logger.debug("The value of the cancelJobOnError of the ProActive MapReduce ReducerPATask is '"
						+ paMapReduceJobConfiguration
								.getCancelJobOnError(PAMapReduceFramework.REDUCER_PA_TASK)
						+ "'");
				...
				logger.debug("The value of the maxNumberOfExecutions of the ProActive MapReduce ReducerPATask is '"
						+ paMapReduceJobConfiguration
								.getMaxNumberOfExecutions(PAMapReduceFramework.REDUCER_PA_TASK)
						+ "'");
				...
				reducerPATask
						.setPreciousLogs(PAMapReduceFrameworkProperties
								.getPropertyAsBoolean(PAMapReduceFrameworkProperties.WORKFLOW_JAVA_TASK_LOGGING_DEBUG.key));
				logger.debug("The value of the precious logs of the ProActive MapReduce ReducerPATask is '"
						+ PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.WORKFLOW_JAVA_TASK_LOGGING_DEBUG.key)
						+ "'");
				...
				reducerPATask
						.addArgument(
								...
								paMapReduceJobConfiguration
										.getReadMode(PAMapReduceFramework.REDUCER_PA_TASK).key);
				logger.debug("The value of the readMode of the ProActive MapReduce ReducerPATask is '"
						+ paMapReduceJobConfiguration
								.getReadMode(PAMapReduceFramework.REDUCER_PA_TASK)
						+ "'");
				...
				reducerPATask
						.addArgument(
								...
								paMapReduceJobConfiguration
										.getInputSpace(PAMapReduceFramework.REDUCER_PA_TASK));
				...
				logger.debug("The value of the intermediateIndexFileSelector of the ProActive MapReduce ReducerPATask is '"
				...
				if (paMapReduceJobConfiguration.getReadMode(
						PAMapReduceFramework.REDUCER_PA_TASK).equals(
						ReadMode.fullLocalRead)) {
					/*
					 * the string to use to select the output files of a mapper
					 * is the following: intermediate_*.out
					 */
					reducerInputFileSelector = PAMapReduceFramework
							.getReducerIntermediateFileSelector();
					logger.debug("The value of the intermediateFileSelector of the ProActive MapReduce ReducerPATask is '"
				...
				reducerPATask
						.addArgument(
								...
								paMapReduceJobConfiguration
										.getOutputSpace(PAMapReduceFramework.REDUCER_PA_TASK));
				...
				String outputFilesSelectionString = paHadoopJobConfiguration
						...
						+ PAMapReduceFramework
								.getTemporaryOutputDirectoryRegex();
				logger.debug("The value of the outputFileSelectionString of the ProActive MapReduce ReducerPATask is '"
				 ...
				 */
				if (paMapReduceJobConfiguration.getWriteMode(
						PAMapReduceFramework.REDUCER_PA_TASK).equals(
				...
				logger.debug("The value of the outputAccessMode of the ProActive MapReduce ReducerPATask is '"
						+ paMapReduceJobConfiguration
								.getOutputAccessMode(PAMapReduceFramework.REDUCER_PA_TASK)
						+ "'");
				...
				logger.debug("The value of the writeMode of the ProActive MapReduce ReducerPATask is '"
						+ paMapReduceJobConfiguration
								.getWriteMode(PAMapReduceFramework.REDUCER_PA_TASK)
						+ "'");
				...
				reducerJoinPATask
						.setName(paMapReduceJobConfiguration
				...
				logger.debug("The value of the description of the ProActive MapReduce ReducerJoinPATask is '"
						+ paMapReduceJobConfiguration
								.getDescription(PAMapReduceFramework.REDUCER_JOIN_PA_TASK)
						+ "'");
				...
				reducerJoinPATask
						.setPreciousLogs(PAMapReduceFrameworkProperties
								.getPropertyAsBoolean(PAMapReduceFrameworkProperties.WORKFLOW_JAVA_TASK_LOGGING_DEBUG.key));
				logger.debug("The value of the precious logs of the ProActive MapReduce ReducerJoinPATask is '"
						+ PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.WORKFLOW_JAVA_TASK_LOGGING_DEBUG.key)
						+ "'");
				...
				logger.debug("The value of the cancelJobOnError of the ProActive MapReduce ReducerJoinPATask is '"
						+ paMapReduceJobConfiguration
								.getCancelJobOnError(PAMapReduceFramework.REDUCER_JOIN_PA_TASK)
						+ "'");
				...
				logger.debug("The value of the restartTaskOnError of the ProActive MapReduce ReducerJoinPATask is '"
						+ paMapReduceJobConfiguration
								.getRestartMode(PAMapReduceFramework.REDUCER_JOIN_PA_TASK)
						+ "'");
				...
				logger.debug("The value of the maxNumberOfExecutions of the ProActive MapReduce ReducerJoinPATask is '"
						+ paMapReduceJobConfiguration
								.getMaxNumberOfExecutions(PAMapReduceFramework.REDUCER_JOIN_PA_TASK)
						+ "'");
				...
				logger.debug("The value of the executableClassName of the ProActive MapReduce ReducerJoinPATask is '"
						+ PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.WORKFLOW_REDUCER_JOIN_PA_TASK_EXECUTABLE_CLASS.key)
						+ "'");
				...
				InputStream fileTransferPostScriptInputStream = Thread
						.currentThread()
						.getContextClassLoader()
						.getResourceAsStream(
				...
				try {
					...
					if (reducerOutputFileNamePrefix == null) {
						reducerOutputFileNamePrefix = PAMapReduceFramework
								.getReducerOutputFileNamePrefix();
					}
	...
	protected boolean submitJob(TaskFlowJob mapReduceWorkflow) {
		...
		try {
			sai = SchedulerConnection
					.join(paMapReduceJobConfiguration
		...
		try {
			...
			try {
				...
				if (pubKey == null) {
					pubKey = Credentials.getPublicKey(Credentials
							.getPubKeyPath());
				}
				try {
					...
					scheduler = sai.login(Credentials.createCredentials(
							new CredData(username, password), pubKey));
				} catch (LoginException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
	 ...
	 * @throws PAJobConfigurationException
	 */
	protected PAHadoopJobConfiguration getPAHadoopJobConfiguration(
			Configuration configuration) throws PAJobConfigurationException {
		if (configuration != null) {
			changeHadoopInputPathList(configuration);
			changeHadoopOutputPath(configuration);

			PAHadoopJobConfiguration pahjc = new PAHadoopJobConfiguration(
					configuration);
			...
			if (paMapReduceJobConfiguration.getInputSplitSize() != Long
					.parseLong(PAMapReduceFramework.getDefault(PAMapReduceFrameworkProperties
				...
				logger.debug("The value to use to set the minimum size for the input split is '"
						+ pahjc.getLong(
								PAMapReduceFrameworkProperties
										.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_MIN_SPLIT_SIZE_PROPERTY_NAME
												.getKey()),
								Long.parseLong(PAMapReduceFramework
										.getDefault(PAMapReduceFrameworkProperties
												.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_MIN_SPLIT_SIZE_PROPERTY_NAME
														.getKey())))) + "'");
				logger.debug("The default value of the minimum size of the input split is '"
						+ Long.parseLong(PAMapReduceFramework.getDefault(PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_MIN_SPLIT_SIZE_PROPERTY_NAME
										.getKey()))) + "'");
				...
				if (pahjc
					...
					logger.debug("The minimum size of the input split in the ProActive MapReduce job is '"
							+ pahjc.get(PAMapReduceFrameworkProperties
									.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_MIN_SPLIT_SIZE_PROPERTY_NAME
											.getKey())) + "'");
				}

				logger.debug("The value to use to set the maximum size for the input split is '"
						+ pahjc.getLong(
								PAMapReduceFrameworkProperties
										.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_MAX_SPLIT_SIZE_PROPERTY_NAME
												.getKey()),
								Long.parseLong(PAMapReduceFramework
										.getDefault(PAMapReduceFrameworkProperties
												.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_MAX_SPLIT_SIZE_PROPERTY_NAME
														.getKey())))) + "'");
				logger.debug("The default value of the maximum size of the input split is '"
						+ Long.parseLong(PAMapReduceFramework.getDefault(PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_MAX_SPLIT_SIZE_PROPERTY_NAME
										.getKey()))) + "'");
				...
				if (pahjc
					...
					logger.debug("The maximum size of the input split in the ProActive MapReduce job is '"
							+ pahjc.get(PAMapReduceFrameworkProperties
									.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_MAX_SPLIT_SIZE_PROPERTY_NAME
											.getKey())) + "'");
				}
	...
	protected void changeHadoopInputPathList(Configuration configuration) {
		String inputPathStringList = configuration
				.get(PAMapReduceFrameworkProperties
						.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_INPUT_DIRECTORY_PROPERTY_NAME.key));
		if ((inputPathStringList != null)
				&& (!inputPathStringList.trim().equalsIgnoreCase(""))) {
			String newInputPathStringList = "";
			String[] list = StringUtils.split(inputPathStringList);
			for (int i = 0; i < list.length; i++) {
				if (i == 0) {
					newInputPathStringList += changeHadoopPath(StringUtils
							.escapeString(list[i]));
				} else {
					newInputPathStringList += StringUtils.COMMA_STR
							+ changeHadoopPath(StringUtils
									.escapeString(list[i]));
				}
			}
			configuration
	...
	protected void changeHadoopOutputPath(Configuration configuration) {
		...
		if ((outputPathString != null)
				&& (!outputPathString.trim().equalsIgnoreCase(""))) {
			outputPathString = changeHadoopPath(StringUtils
					.escapeString(outputPathString));
			configuration
					.set(PAMapReduceFrameworkProperties
							.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_OUTPUT_DIRECTORY_PROPERTY_NAME.key),
							outputPathString);
	...
	protected String changeHadoopPath(String pathString) {
		...
		if ((colon != -1) && ((slash == -1) || (colon < slash))) { // has a
			...
			String userDirString = System
					.getProperty(PAMapReduceFramework.USER_DIR);
			...
			if (startOfUserDirString > 0) {
				 ...
				 */
				String afterUserDirString = pathString
						.substring(startOfUserDirString
	...
	protected boolean hasWindowsDrive(String pathString, boolean slashed) {
		...
		return pathString.length() >= start + 2
				...
				&& ((pathString.charAt(start) >= 'A' && pathString
						.charAt(start) <= 'Z') || (pathString.charAt(start) >= 'a' && pathString
						.charAt(start) <= 'z'));
	}
	...
	protected JobPriority getPriority(String hadoopJobPriorityString) {
		if ((hadoopJobPriorityString == null)
				|| (hadoopJobPriorityString.trim().equalsIgnoreCase(""))) {
			return JobPriority.NORMAL;
		} else {
			if (org.apache.hadoop.mapred.JobPriority.valueOf(
					hadoopJobPriorityString).equals(
					org.apache.hadoop.mapred.JobPriority.VERY_HIGH)) {
				return JobPriority.HIGHEST;
			} else if (org.apache.hadoop.mapred.JobPriority.valueOf(
					hadoopJobPriorityString).equals(
	...
	protected boolean checkConfiguration(PAMapReduceJobConfiguration pamrjc,
			List<String> requiredConfigurationPropertyList)
			throws PAJobConfigurationException {
		for (String currentProperty : requiredConfigurationPropertyList) {
			if (pamrjc.getPropertyAsString(currentProperty) == null) {
				throw new PAJobConfigurationException("Property '"
						+ currentProperty + "' is required but it is not set!");
			}
		}
	...
	protected List<String> initRequiredConfigurationProperties() {
		...
		requiredConfigurationPropertyList
				.add(PAMapReduceFrameworkProperties.SCHEDULER_HOME.getKey());
		 ...
		 */
		requiredConfigurationPropertyList
				.add(PAMapReduceFrameworkProperties.WORKFLOW_INPUT_SPACE
						.getKey());
		 ...
		 */
		requiredConfigurationPropertyList
				.add(PAMapReduceFrameworkProperties.WORKFLOW_MAPPER_PA_TASK_READ_MODE
						.getKey());
		 ...
		 */
		requiredConfigurationPropertyList
				.add(PAMapReduceFrameworkProperties.WORKFLOW_REDUCER_PA_TASK_READ_MODE
						.getKey());
		 ...
		 */
		requiredConfigurationPropertyList
				.add(PAMapReduceFrameworkProperties.WORKFLOW_MAPPER_PA_TASK_WRITE_MODE
						.getKey());
		 ...
		 */
		requiredConfigurationPropertyList
				.add(PAMapReduceFrameworkProperties.WORKFLOW_REDUCER_PA_TASK_WRITE_MODE
						.getKey());
