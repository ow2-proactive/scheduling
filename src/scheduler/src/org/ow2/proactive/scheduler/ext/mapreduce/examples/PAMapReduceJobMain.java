...
public class PAMapReduceJobMain {
	...
	public static void main(String[] args) throws IOException,
			URISyntaxException, PAJobConfigurationException {
		...
		try {
			...
			try {
				job.setNumReduceTasks(Integer
						.parseInt(args[numberOfReducersArgumentIndex]));
			...
			FileInputFormat.addInputPath(job, new Path(
					args[inputFilePathArgumentIndex]));
			...
			FileOutputFormat.setOutputPath(job, new Path(
					args[outputFolderPathArgumentIndex]));
		...
		if (paMapReduceConfigurationFileArgumentIndex >= 0) {
			File paMapReduceConfigurationFile = new File(
					args[paMapReduceConfigurationFileArgumentIndex]);
	 ...
	 * in Hadoop 0.20.2
	 */
	public static class IntSumReducerCombiner extends
			Reducer<Text, IntWritable, Text, IntWritable> {
		private IntWritable result = new IntWritable();
		...
		public void run(Context context) throws IOException,
			...
			while (context.nextKey()) {
				reduce((Text) context.getCurrentKey(), context.getValues(),
						context);
			}
			cleanup(context);
		}
	 ...
	 */
	public static class TalendSortLineitemShipModeMapper extends
	 ...
	 */
	public static class TalendSortLineitemShipModeReducer extends
