...
public class Merger {
	...
	public static <K extends Object, V extends Object> RawKeyValueIterator merge(
			...
			Class<V> valueClass, Path[] inputs, boolean deleteInputs,
			int mergeFactor, Path tmpDir, RawComparator<K> comparator)
			throws IOException {
		return new MergeQueue<K, V>(conf, fs, inputs, deleteInputs, comparator)
	...
	public static <K extends Object, V extends Object> RawKeyValueIterator merge(
			...
			Path tmpDir, RawComparator<K> comparator) throws IOException {
		return new MergeQueue<K, V>(conf, fs, segments, comparator, false)
				.merge(keyClass, valueClass, mergeFactor, tmpDir);
	...
	public static <K extends Object, V extends Object> RawKeyValueIterator merge(
			Configuration conf, FileSystem fs, Class<K> keyClass,
			Class<V> valueClass, List<Segment<K, V>> segments, int mergeFactor,
			Path tmpDir, RawComparator<K> comparator, boolean sortSegments)
			throws IOException {
		return new MergeQueue<K, V>(conf, fs, segments, comparator,
	...
	static <K extends Object, V extends Object> RawKeyValueIterator merge(
			...
			boolean sortSegments) throws IOException {
		return new MergeQueue<K, V>(conf, fs, segments, comparator,
				sortSegments).merge(keyClass, valueClass, mergeFactor,
				inMemSegments, tmpDir);
	}

	public static <K extends Object, V extends Object> void writeFile(
			RawKeyValueIterator records, IntermediateFile.Writer<K, V> writer,
	...
	public static class Segment<K extends Object, V extends Object> {
		...
		public Segment(Configuration conf, FileSystem fs, Path file,
				long segmentOffset, long segmentLength, boolean preserve)
				throws IOException {
			this.conf = conf;
			this.fs = fs;
			this.file = file;
		...
		private void init() throws IOException {
			if (reader == null) {
				...
				reader = new IntermediateFile.Reader<K, V>(conf, in,
						segmentLength);
			}
		}
	...
	private static class MergeQueue<K extends Object, V extends Object> extends
		...
		public MergeQueue(Configuration conf, FileSystem fs, Path[] inputs,
				boolean deleteInputs, RawComparator<K> comparator)
				throws IOException {
			this.conf = conf;
			this.fs = fs;
		...
		RawKeyValueIterator merge(Class<K> keyClass, Class<V> valueClass,
				int factor, int inMem, Path tmpDir) throws IOException {
			...
			do {
				...
				if (numSegments <= factor) {
					...
					Path outputFile = new Path(
							PAMapReduceFramework.MAPPER_INTERMEDIATE_FILE_NAME_PREFIX
									+ PAMapReduceFramework.FILE_EXTENSION_SEPARATOR
									+ passNo);

					IntermediateFile.Writer<K, V> writer = new IntermediateFile.Writer<K, V>(
							conf, fs, outputFile, keyClass, valueClass);
