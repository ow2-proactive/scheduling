...
public class SplitterPATask extends JavaExecutable {
	...
	public void init(Map<String, Serializable> args) throws Exception {
		...
		String inputDataSpaceString = (String) args
				.get(PAMapReduceFrameworkProperties.WORKFLOW_SPLITTER_PA_TASK_INPUT_DATASPACE
						.getKey());
		...
		if (inputDataSpaceString
			...
			inputDataSpace = getInputSpace();
				...
				.equalsIgnoreCase(PAMapReduceFrameworkProperties.WORKFLOW_JAVA_TASK_LOCAL_DATASPACE
						.getKey())) {
			inputDataSpace = getLocalSpace();
		} else {
			inputDataSpace = getOutputSpace();
		}
		logger.debug("The real URI of the data space to use as input data space is: " + inputDataSpace.getRealURI());
		logger.debug("The virtual URI of the data space to use as the input data space is: " + inputDataSpace.getVirtualURI());
		...
		String inputPathStringList = hadoopJobConfiguration
		...
		logger.debug("The comma separated list of files received is "
				+ inputPathStringList);

		if ((inputPathStringList != null)
				&& (!inputPathStringList.trim().equalsIgnoreCase(""))) {
			String newInputPathStringList = "";
			String[] inputPathStringListElments = StringUtils
					.split(inputPathStringList);
			Path currentPath = null;
			FileStatus[] currentFileStatusArray = null;
			for (int i = 0; i < inputPathStringListElments.length; i++) {
				currentPath = new Path(
						StringUtils.escapeString(inputPathStringListElments[i]));

				logger.debug("Current path is: " + currentPath);
				FileStatus currentPathFileStatus = inputFileSystem.getFileStatus(currentPath);
				logger.debug("Current path file status information: " + currentPathFileStatus.getPath().toUri().toString());
				String inputFileSystemWorkingDirectory = inputFileSystem.getWorkingDirectory().toUri().toString();
				...
				if (inputFileSystem.getFileStatus(currentPath).isDir()) {
					currentFileStatusArray = inputFileSystem.listStatus(
							currentPath, new ExcludeHiddenFilePathFilter(
									inputFileSystem));

					logger.debug("The number of children of the directory '"
							+ currentPath.toUri().toString() + "' is  "
							+ currentFileStatusArray.length + "'");
					...
					for (int j = 0; j < currentFileStatusArray.length; j++) {
						...
						if (!currentFileStatusArray[j].isDir()) {
							if (j == 0) {
								newInputPathStringList += currentFileStatusArray[j]
										.getPath().toUri().toString();
							} else {
								newInputPathStringList += StringUtils.COMMA_STR
										+ currentFileStatusArray[j].getPath()
												.toUri().toString();
							}
						}
					}
					...
					if (i == 0) {
						...
						newInputPathStringList += StringUtils.COMMA_STR
								+ inputPathStringListElments[i];
					}
				}
			}

			logger.debug("The modified list of input paths is '"
			...
			logger.debug("The list of modified input files is "
					+ hadoopJobConfiguration.get(PAMapReduceFrameworkProperties
							.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_INPUT_DIRECTORY_PROPERTY_NAME
									.getKey())));
		}
	...
	public Serializable execute(TaskResult... taskResults) throws Throwable {
		if (hadoopInputFormat != null) {
			...
			JobContext fakeHadoopJobContext = new FakeHadoopJobContext(
					hadoopJobConfiguration, hadoopJobId);
			 ...
			 */
			List<InputSplit> inputSplitList = hadoopInputFormat
					.getSplits(fakeHadoopJobContext);
			ArrayList<InputSplit> inputSplitArrayList = new ArrayList<InputSplit>(
					inputSplitList.size());
			for (InputSplit inputSplit : inputSplitList) {
				inputSplitArrayList.add(new SerializableHadoopInputSplit(
						inputSplit, inputSplit.getClass()));
			}

			logger.debug("The number of created splits is: "
					+ inputSplitArrayList.size());
