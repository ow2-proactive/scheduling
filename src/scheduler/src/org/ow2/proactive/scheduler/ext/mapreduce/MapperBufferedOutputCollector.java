 ...
 */
public class MapperBufferedOutputCollector<K, V> implements
		MapperOutputCollector<K, V>, IndexedSortable {
	 ...
	 * @throws IOException
	 */
	public MapperBufferedOutputCollector(TaskAttemptContext taskAttemptContext,
				...
				.getFloat(
						PAMapReduceFrameworkProperties
								.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_IO_SORT_RECORD_PERCENT_PROPERTY_NAME
										.getKey()), (float) 0.05);
		final int sortmb = configuration
				.getInt(PAMapReduceFrameworkProperties
						...
						configuration.getClass(
								PAMapReduceFrameworkProperties
										.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_IO_SORT_MAP_SORT_CLASS_PROPERTY_NAME
												.getKey()), QuickSort.class,
								IndexedSorter.class), configuration);
		...
		valClass = (Class<V>) ((PAHadoopJobConfiguration) configuration)
	...
	public synchronized void flush() throws IOException, InterruptedException,
		...
		logger.profile("End of the [MAPPER_MERGING_PHASE]. It takes '"
				+ (mapperMergeEndTime - mapperMergeStartTime)
				+ "' milliseconds");
		...
		/** @START_PROFILE */
		logger.profile("End of the [MAPPER_COMBINING_PHASE]. It takes '"
				+ combinerExecutionTime + "' milliseconds");
		/** @END_PROFILE */
	}
	...
	protected void mergeParts() throws IOException, InterruptedException,
		...
		final String mapperId = configuration
				.get(PAMapReduceFrameworkProperties.PA_MAPREDUCE_TASK_IDENTIFIER
						.getKey());
		...
		if (numSpills == 1) { // the spill is the final output
			...
			if (indexCacheList.size() == 0) {
				fileSystem
						.rename(getSpillIndexFile(mapperId, "" + 0),
								new Path(
										PAMapReduceFramework
		 ...
		 */
		Path finalOutputFile = new Path(
				PAMapReduceFramework.getMapperIntermediateFileName(configuration
						.get(PAMapReduceFrameworkProperties.PA_MAPREDUCE_TASK_IDENTIFIER
								.getKey())));
		Path finalIndexFile = new Path(
				PAMapReduceFramework.getMapperIntermediateIndexFileName(configuration
						.get(PAMapReduceFrameworkProperties.PA_MAPREDUCE_TASK_IDENTIFIER
								.getKey())));

		// The output stream for the final single output file
		FSDataOutputStream finalOut = fileSystem.create(finalOutputFile, true,
				4096);
		...
		if (numSpills == 0) {
			...
			try {
				for (int i = 0; i < partitions; i++) {
					...
					Writer<K, V> writer = new Writer<K, V>(configuration,
							finalOut, keyClass, valClass);
					writer.close();
		...
		}
		{
			...
			for (int parts = 0; parts < partitions; parts++) {
				...
				List<Merger.Segment<K, V>> segmentList = new ArrayList<Merger.Segment<K, V>>(
						numSpills);
				for (int i = 0; i < numSpills; i++) {
					IndexRecord indexRecord = indexCacheList.get(i).getIndex(
							parts);
					Merger.Segment<K, V> s = new Merger.Segment<K, V>(
							configuration, fileSystem, filename[i],
								...
								configuration.getInt(
										PAMapReduceFrameworkProperties
												.getPropertyAsString(PAMapReduceFrameworkProperties.HADOOP_IO_SORT_FACTOR_PROPERTY_NAME
														.getKey()), 100),
								new Path(mapperId),
								((PAHadoopJobConfiguration) configuration)
										.getOutputKeyComparator());

				// write merged output to disk
				long segmentStart = finalOut.getPos();
				Writer<K, V> writer = new Writer<K, V>(configuration, finalOut,
						keyClass, valClass);
	...
	protected Path getSpillFile(String mapperId, String spillId) {
		return new Path(
				PAMapReduceFramework.getSpillFileName(mapperId, spillId));
	}
	...
	protected Path getSpillIndexFile(String mapperId, String spillId) {
		return new Path(PAMapReduceFramework.getSpillIndexFileName(mapperId,
				spillId));
	}
					...
					if (kvstart == kvend) {
						...
						if (kvend != kvindex) {
							...
							final int size = ((bufend <= bufindex) ? bufindex
									- bufend : (bufvoid - bufend) + bufindex)
									+ len;
							bufstart = bufend = bufindex = bufmark = 0;
							kvstart = kvend = kvindex = 0;
							bufvoid = kvbuffer.length;
							throw new IOException(
									"The collection buffer is too small "
											+ size + " bytes");
						}
					}
	...
	protected void sortAndSpill() throws IOException, ClassNotFoundException,
		...
		try {
			 ...
			 */
			String mapperId = configuration
					.get(PAMapReduceFrameworkProperties.PA_MAPREDUCE_TASK_IDENTIFIER
							.getKey());
			final Path filename = new Path(
					PAMapReduceFramework.getSpillFileName(mapperId, ""
							+ numSpills));
			out = fileSystem.create(filename);

			final int endPosition = (kvend > kvstart) ? kvend
					: kvoffsets.length + kvend;
			sorter.sort(MapperBufferedOutputCollector.this, kvstart,
					endPosition);
			int spindex = kvstart;
			IndexRecord rec = new IndexRecord();
			...
			for (int i = 0; i < partitions; ++i) {
				...
				try {
					...
					writer = new Writer<K, V>(configuration, out, keyClass,
							valClass);
					if (combinerRunner == null) {
						...
						while (spindex < endPosition
							...
							key.reset(
									...
									(kvindices[kvoff + VALSTART] - kvindices[kvoff
											+ KEYSTART]));
			...
			if (totalIndexCacheMemory >= INDEX_CACHE_MEMORY_LIMIT) {
				...
				Path indexFilename = new Path(
						PAMapReduceFramework.getSpillIndexFileName(mapperId, ""
								+ numSpills));
	...
	public int compare(int i, int j) {
		...
		return comparator.compare(kvbuffer, kvindices[ii + KEYSTART],
				kvindices[ii + VALSTART] - kvindices[ii + KEYSTART], kvbuffer,
				kvindices[ij + KEYSTART], kvindices[ij + VALSTART]
	...
	protected class MRResultIterator implements RawKeyValueIterator {
		...
		public DataInputBuffer getKey() throws IOException {
			...
			keybuf.reset(kvbuffer, kvindices[kvoff + KEYSTART], kvindices[kvoff
					+ VALSTART]
					- kvindices[kvoff + KEYSTART]);
			return keybuf;
		}
