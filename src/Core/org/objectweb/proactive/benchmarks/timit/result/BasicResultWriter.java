...
public class BasicResultWriter {
	...
	private void fillTimersResults(final Element rootElement, final ResultBag bag){
		...
		rootElement.setAttribute(new Attribute("proActiveVersion", ProActive.getProActiveVersion()));
	    ...
	    while( timersListIterator.hasNext() ){
	    	...
	    	if ( currentTimer.getParent() == null ){	    		
	    		...
	    		while ( descendantsIterator.hasNext() ){
	    			...
	    			// Match the parent
	    			if ( parentElement.getAttributeValue("name").equals(currentTimer.getParent().getName()) ){	    				
	    				parentElement.addContent(createTimerElement(currentTimer));
	    				break;
	    			}	    			
	    		}
	    	}	    				    
	...
	private static final Element createTimerElement(final BasicTimer currentTimer){
		...
		newTimerElement.setAttribute(new Attribute("totalTimeInMillis", ""+((double)currentTimer.getTotalTime())/1000000d));
		// Set the number of startStopCoupleCount value
		newTimerElement.setAttribute(new Attribute("startStopCoupleCount", ""+currentTimer.getStartStopCoupleCount()));
