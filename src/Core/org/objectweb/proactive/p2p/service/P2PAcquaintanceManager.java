...
public class P2PAcquaintanceManager implements InitActive, RunActive,
	...
	private final static Logger logger = ProActiveLogger
			.getLogger(Loggers.P2P_ACQUAINTANCES);
	static public int NOA = Integer
			.parseInt(PAProperties.PA_P2P_NOA.getValue());
	...
	public void connectToPreferedAcquaintances() {
		...
		while (it.hasNext() && (this.getEstimatedNumberOfAcquaintances() < NOA)) {
			...
			try {
				...
				P2PService peer = (P2PService) distNode
						.getActiveObjects(P2PService.class.getName())[0];
				if ( // !peer.equals(this.localService) &&
				!this.contains(peer).booleanValue()) {
					// Send a message to the remote peer to register myself
					System.out
							.println("P2PAcquaintanceManager requesting peer "
									+ peerUrl);
	...
	public void cleanAwaitedReplies() {
		...
		while (it.hasNext()) {
			Map.Entry<String, DatedRequest> entry = (Map.Entry<String, DatedRequest>) it
					.next();
	...
	public void startAcquaintanceHandShake(String peerUrl, P2PService peer) {
		...
		awaitedReplies.put(buildCorrectUrl(peerUrl), new DatedRequest(peer,
				System.currentTimeMillis()));
	}
	...
	public Vector<String> add(P2PService peer) {
		return this.add(buildCorrectUrl(PAActiveObject
				.getActiveObjectNodeUrl(peer)), peer);
	}
	...
	public Vector<String> getAcquaintancesURLs() {
		return new Vector<String>(Arrays.asList(this.acquaintances
	...
	protected boolean acceptAnotherAcquaintance() {
		...
		double prob = (-(1.0 / this.getMaxNOA()) * this
				.getEstimatedNumberOfAcquaintances()) + 2;
	...
	public void dumpTables() {
		...
		while (it2.hasNext()) {
			Map.Entry<String, DatedRequest> entry = (Map.Entry<String, DatedRequest>) it2
					.next();
