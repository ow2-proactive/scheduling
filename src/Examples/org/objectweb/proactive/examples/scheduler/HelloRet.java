...
public class HelloRet {
    public static void main(String[] args) {
        ...
        try {
            ...
            String tID;
            System.out.println(
                "Please enter the task id to get it and 'stop' to ecit");
            while (!(tID = buf.readLine()).equals("stop")) {
                if (scheduler.isFinished(tID).booleanValue()) {
                    InternalResult result = scheduler.getResult(tID,
                            System.getProperty("user.name"));
                    if (result.getErrorMessage().equals("")) {
                        System.out.println(result.getProActiveTaskExecutionResult()
                                                 .getObject());
                    } else {
                    ...
                    System.out.println("Not finished or doesnt exist");
                }
            }
        } catch (Exception e) {
