...
public class Agent implements org.objectweb.proactive.RunActive, Serializable {
    ...
    private String getNextDestination() {
        String newString;
        do {
            newString = (String) destinationList.elementAt(randomGenerator.nextInt(
                        maxDestination));
    ...
    private void chooseValue() {
        ...
        System.out.println("Agent " + id + ": chooseValue() value = " + value +
            " min = " + min);
    ...
    public void notifyOthers() {
        ...
        for (int j = 0; j < max; j++) {
            ((Agent) otherList.elementAt(j)).newMinimum(min,
                new String("" + id + "-" + messageSequenceNumber));
