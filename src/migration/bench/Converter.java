...
public class Converter {
  ...
  public static File findFile(String fileName) {
    ...
    if (theFile.exists()) {
      if (theFile.canRead()) {
        System.out.println("File " + theFile.getAbsolutePath() + " correctly found");
        return theFile;
      } else {
        System.err.println("Cannot read file " + fileName + ", although file exists.");
        System.err.println("Hint: check file access rights.");
        return null;
      }
    } else {
      System.err.println("File " + fileName + " does not exist.");
      System.err.println("Hint: absolute path is " + theFile.getAbsolutePath() + ".");
      return null;
    }
  }
  ...
  public static void main(String args[]) {
    if (args.length < 1) {
      System.out.println("Usage: java migration.bench.Converter rmi://hostname/objectName");
      System.exit(-1);
    }
    try {
      //First we create a new active object
      Converter converter = (Converter)ProActive.newActive("migration.bench.Converter", null);
      //Then we register
