...
public class ImageBlaster extends Canvas implements Runnable {
  ...
  public void run() {
    long begin;            /**Begin the movie (use for Benchmark)*/
    long finish;           /**Stop the movie (use for Benchmark)*/
    long spaceTime;        /**movie's space time (use for Benchmark)*/
    long space1;
    long space2;
    long waiting;
    ...
    while (then) {
      while (animator == me) {
        ...
        try {
          ...
          System.out.println("InterruptedException in ImageBlaster " + e);
          return;
        }
        try {
          Thread.sleep(40);
        } catch (InterruptedException e) {
          System.out.println("InterruptedException in ImageBlaster.java " + e);
          break;
        }
