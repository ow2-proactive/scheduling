...
public class Client extends StreamClient
    ...
    public Client(Integer n, String servers) {
        ...
        while (st.hasMoreTokens()) {
            try {
                ...
                s = (Server) ProActive.lookupActive(Server.class.getName(),
                        "//" + name);
                ...
                System.out.println("\nFailed to lookup server on host //" +
                    name);
                System.exit(1);
            }
        }
    ...
    public void registerSelf() {
        ...
        if (myself != null) {
            for (Enumeration e = playerArray.elements(); e.hasMoreElements();
                    i++)
                ((StreamServer) ((Player) e.nextElement()).server).registerClient(myself,
                    i);
        } else {
    ...
    public static void main(String[] args) {
        ...
        if (n < 1) {
            System.out.println(
                "Syntax: java Client <serverhostname> [serverhostname2] [...]");
            return;
        }
        ...
        try {
            ...
            Client theClient = (Client) ProActive.newActive("essai.proplayer.Client",
                    params, NodeFactory.getNode("//localhost:7777/Node1"));
