...
public class ProActiveComponentImpl implements ProActiveComponent, Interface, Serializable {
	...
	public ProActiveComponentImpl(ComponentParameters componentParameters, Body myBody) {
		...
		ComponentParametersController component_parameters_controller =
			new ProActiveComponentParametersController(this);
		...
		if (!(componentParameters.getHierarchicalType().equals(ComponentParameters.PRIMITIVE) && componentParameters.getClientInterfaceTypes().length == 0)) {
			interface_references_vector.add(new ProActiveBindingController(this));
		} else {
			//bindingController = null;
			if (logger.isDebugEnabled()) {
				logger.debug(
					"user component class of '"
		...
		if (componentParameters.getHierarchicalType().equals(ComponentParameters.COMPOSITE)
			...
			// add and resize
			interface_references_vector.addElement(new ProActiveContentController(this));
		} 
		...
		try {
			for (int i = 0; i < interface_types.length; i++) {
				...
				if (interface_types[i].isFcClientItf() && interface_types[i].isFcCollectionItf()) {
					//Component itfg = ProActiveComponentGroup.newActiveComponentGroup(interface_types[i]);
					itf_ref = createInterfaceOnGroupOfDelegatees(interface_types[i], component_is_primitive);
				}
				// if we have a PARALLEL component, we also create a group proxy on the delegatee field
					...
					if (!interface_types[i].isFcClientItf()) {
						itf_ref =
							createInterfaceOnGroupOfDelegatees(interface_types[i], component_is_primitive);
					}
				} else {
					itf_ref =
						MetaObjectInterfaceClassGenerator.instance().generateInterface(
							interface_types[i].getFcItfName(),
							this,
							interface_types[i],
							true,
		...
		// put all in a table
		interfaceReferences =
			(Interface[]) interface_references_vector.toArray(
				new Interface[interface_references_vector.size()]);
	}
					...
					if (count > 0) {
						if (count == itf_ref_group.size()) {
							...
							throw new NoSuchInterfaceException(
								"some elements of the collection are not named " + interfaceName);
						}
					}
				} else {
	...
	public Type getFcType() {
		try {
		return (
			(ComponentParametersController) getFcInterface(Constants
				.COMPONENT_PARAMETERS_CONTROLLER))
			.getComponentParameters()
			.getComponentType();
		} catch (NoSuchInterfaceException nsie) {
			nsie.printStackTrace();
			throw new ProActiveRuntimeException("cannot retreive the type of the component", nsie);
		}
	}
	...
	public boolean equals(Component componentIdentity) {
		try {
		return (
			(ComponentParametersController) componentIdentity.getFcInterface(
