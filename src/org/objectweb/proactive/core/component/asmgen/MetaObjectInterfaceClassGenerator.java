...
public class MetaObjectInterfaceClassGenerator extends AbstractInterfaceClassGenerator {
	...
	public ProActiveInterface generateInterface(
		...
		this.stubClassFullName =
			org.objectweb.proactive.core.component.asmgen.Utils.getMetaObjectClassName(
				fcInterfaceName,
		...
		try {
			...
			if (logger.isDebugEnabled()) {
				...
				logger.debug("generated classes cache is : " + getGeneratedClassesCache().toString());
			}
	...
	protected void createGetAndSetFcItfImplMethods() {
		...
		CodeVisitor cv = this.classGenerator.visitMethod(ACC_PUBLIC, "getFcItfImpl", "()" + OBJECT_TYPE, null);

		// Now, fills in the instruction list
		cv.visitVarInsn(ALOAD, 0);
		cv.visitFieldInsn(GETFIELD, this.stubClassFullName.replace('.', '/'), IMPL_FIELD_NAME, OBJECT_TYPE);
		...
		cv = this.classGenerator.visitMethod(ACC_PUBLIC, "setFcItfImpl", "(" + OBJECT_TYPE + ")V", null);
		...
		cv.visitFieldInsn(PUTFIELD, this.stubClassFullName.replace('.', '/'), IMPL_FIELD_NAME, OBJECT_TYPE);
		cv.visitInsn(RETURN);
	...
	protected void createFields() {
		...
		this.classGenerator.visitField(ACC_PROTECTED, IMPL_FIELD_NAME, OBJECT_TYPE, null);
	}
