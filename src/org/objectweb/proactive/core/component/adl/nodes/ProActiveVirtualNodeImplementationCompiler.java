 ...
 */
public class ProActiveVirtualNodeImplementationCompiler extends ImplementationCompiler {
	public AbstractInstanceProviderTask newCreateTask(final List path, final ComponentContainer container,
			final String name, final String definition, final Object controller, final Object implementation,
			final Map context) {
		VirtualNode n = null;
		if (container instanceof VirtualNodeContainer) {
			...
			if (implementation == null) {
				// a composite (or parallel) component TODO : parallel component
				if (controller.equals("composite")) {
					...
					System.out
							.println("Error while parsing the ADL : if no implementation is specified, a component should have the controller value set either to composite or to parallel");
					controllerDesc = null;
				}
			} else {
			...
			if (n != null) {
				if (context.get("deployment-descriptor") != null) {
					org.objectweb.proactive.core.descriptor.data.VirtualNode vn = ((ProActiveDescriptor) context
							.get("deployment-descriptor")).getVirtualNode(n.getName());
					contentDesc.setVirtualNode(vn);
					return new RemoteCreateTask(builder, name, definition, controllerDesc, contentDesc, context.get(n
							.getName()));
				}
			}
		}
		return super.newCreateTask(path, container, name, definition, controller, implementation, context);
	}
	static class RemoteCreateTask extends AbstractInstanceProviderTask {
		...
		Object contentDesc;
		Object node;
		public RemoteCreateTask(final ImplementationBuilder builder, final String name, final String definition,
				final Object controllerDesc, final Object contentDesc, final Object node) {
