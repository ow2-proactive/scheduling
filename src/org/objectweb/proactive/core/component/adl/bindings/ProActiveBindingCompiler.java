	    ...
	    final Map context) throws ADLException 
	  {
	    ...
	    if (container instanceof BindingContainer) {
	      ...
	      for (int i = 0; i < bindings.length; i++) {
	        ...
	        InstanceProviderTask createClientTask = 
	          (InstanceProviderTask)tasks.getTask("create", clientComp);
	        InstanceProviderTask createServerTask = 
	          (InstanceProviderTask)tasks.getTask("create", serverComp);
	        ...
	        try {
	            ...
	            if ( (BindingBuilder.EXPORT_BINDING == type) 
	                ...
	                for (int j = 0; j < itfs.length; j++) {
	                    ...
	                    if (clientItf.equals(itf.getName())) {
	                        if (ProActiveTypeInterface.MULTICAST_CARDINALITY.equals(itf.getCardinality())) {
              ...
              if ( (BindingBuilder.EXPORT_BINDING == type) 
                  ...
                  clientItf = setMulticastIndex(
                                              clientComp, clientItf, itfContainer);
                  }
