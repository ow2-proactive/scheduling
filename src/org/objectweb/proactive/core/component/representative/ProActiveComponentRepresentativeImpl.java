		...
		if (interfaceName.equals(Constants.BINDING_CONTROLLER)
			...
			for (int i = 0; i < interfaceReferences.length; i++) {
					if (interfaceReferences[i].getFcItfName() != null) {
						if (interfaceReferences[i].getFcItfName().equals(interfaceName) || interfaceName.startsWith(interfaceReferences[i].getFcItfName())) {
							return interfaceReferences[i];
						} 
					}
//				if (!(this == interfaceReferences[i]) && ProActiveGroup.isGroup(interfaceReferences[i])) {
//					Group group = ProActiveGroup.getGroup(interfaceReferences[i]);
//					if (group.containsKey(interfaceName)
//						&& ((InterfaceType) ((Interface) group.get(interfaceName)).getFcItfType()).isFcCollectionItf()) {
//						return group.get(interfaceName);
//					} else {
//						// could be a collective interface with the default name (allowed with ProActive)
//						// get the first element.  Then check its name.
//						if (group.isEmpty()) {
//							// no element. Try to return the collective interface itself
//							// rem : this seems weird, as we are performing a method call on the group itself
//							if (interfaceReferences[i].getFcItfName().equals(interfaceName)) {
//								return interfaceReferences[i];
//							}
//						} else {
//							Interface itf = (Interface) group.iterator().next();
//							if (((InterfaceType) itf.getFcItfType()).getFcItfName().equals(interfaceName)) {
//								return interfaceReferences[i];
//							}
//						}
//					}}
	...
	public String getFcName() {
		return (String) reifyCall(NameController.class.getName(), "getFcName", new Class[] {
		}, new Object[] {
		});
	}
	...
	public Component[] getFcSuperComponents() {
		return (Component[]) reifyCall(SuperController.class.getName(), "getFcSuperComponents", new Class[] {
		}, new Object[] {
		});
	}
