...
public class OARGRIDSubProcess extends AbstractExternalProcessDecorator {
	...
	private static final String DEFAULT_SCRIPT_LOCATION = System
	...
	protected String internalBuildCommand() {
		...
		oarsubCommand
				.append("/bin/sh -c  'echo for i in \\`cat \\$OAR_NODEFILE\\` \\; do "
						+ accessProtocol + " \\$i  ");
	...
	public class OarSite implements Serializable {
		...
		public OarSite(String resource) {
			...
			//ToDo, handle Error case
				...
				System.out
						.println("clusterFrontEndName not found for cluster: "
								+ clusterName);
				if (logger.isDebugEnabled()) {
		...
		public void setResource(String resource) {
			...
			for (int i = 0; i < resTab.length; i++) {
				...
				if (resTab[i].indexOf("=") < 0) //Clustername has no "="
					...
					nodes = Integer.parseInt(resTab[i].substring(resTab[i]
							.indexOf("=") + 1));
				else if (resTab[i].indexOf("weight=") >= 0)
					weight = Integer.parseInt(resTab[i].substring(resTab[i]
							.indexOf("=") + 1));
			}
		}
