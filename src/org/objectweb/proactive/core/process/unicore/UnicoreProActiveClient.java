...
public class UnicoreProActiveClient {
	...
	private void initResourceManager(String keystorefile, String keypassword) {
		...
		try {
			ResourceManager.loadKeystore(keystorefile, keypassword
					.toCharArray());
		} catch (Exception e) {
	...
	private ScriptContainer addScriptTask(String taskName, String scriptContent) {
		...
		ScriptContainer sc = (ScriptContainer) plugableScript
				.getContainerInstance(jc);
	...
	private void setCapResDefaultMaxMin(CapacityResource candidate) {
		NamedResourceSet namedResourceSet = ResourceManager.getResourceSet(jc
				.getVsite());

		CapacityResource cr = (CapacityResource) namedResourceSet
				.findResource(candidate);
	...
	private void buildScriptTask() {
		...
		//*************** Set TASKS *****************
		ScriptContainer sc = addScriptTask("ProActiveTask", uParam
				.getScriptContent());
		...
		/*
		NamedResourceSet siteResources = ResourceManager
		.getResourceSet(jc.getVsite());
		Storage siteStorage = (Storage) siteResources
		.findResource((Storage) storage);
		*/
	...
	private void buildImportFiles(ScriptContainer sc){
		...
		String files[]=uParam.getImportFiles();
		...
		for(int i=0; i<files.length; i++){
			...
			index=files[i].lastIndexOf(uParam.getFileSep());
			
			//File ends with a "/" or "\"
			if(index == files[i].length()-1){
				System.err.println("Skipping: "+files[i]+" directory copy not supported");
				continue;
			}
			else if(index > 0)
				remoteFileName=files[i].substring(index+1);
			else
				remoteFileName=files[i];
			
			//Storage, srcName, srcDest overwrite, isAscii
			FileImport fi = 
				new FileImport("Local", files[i], 
						remoteFileName, true, false);
	...
	public void saveJob() {
		...
		boolean succeeded = ResourceManager.writeObjectToFile(jc, jc
				.getFilename()
				+ ".ajo");
		...
		try {
			...
			XMLObjectWriter dout = new XMLObjectWriter(new File(jc
					.getFilename()
					+ ".xml"));
