...
public class ProcessDefinitionHandler extends AbstractUnmarshallerDecorator
	...
	public ProcessDefinitionHandler(ProActiveDescriptor proActiveDescriptor) {
		...
		this.addHandler(JVM_PROCESS_TAG, new JVMProcessHandler(
				proActiveDescriptor));
		this.addHandler(RSH_PROCESS_TAG, new RSHProcessHandler(
				proActiveDescriptor));
		this.addHandler(MAPRSH_PROCESS_TAG, new MapRshProcessHandler(
				proActiveDescriptor));
		this.addHandler(SSH_PROCESS_TAG, new SSHProcessHandler(
				proActiveDescriptor));
		this.addHandler(RLOGIN_PROCESS_TAG, new RLoginProcessHandler(
				proActiveDescriptor));
		this.addHandler(BSUB_PROCESS_TAG, new BSubProcessHandler(
				proActiveDescriptor));
		this.addHandler(GLOBUS_PROCESS_TAG, new GlobusProcessHandler(
				proActiveDescriptor));
		this.addHandler(PRUN_PROCESS_TAG, new PrunProcessHandler(
				proActiveDescriptor));
		this.addHandler(PBS_PROCESS_TAG, new PBSProcessHandler(
				proActiveDescriptor));
		this.addHandler(GRID_ENGINE_PROCESS_TAG, new GridEngineProcessHandler(
				proActiveDescriptor));
		this.addHandler(OAR_PROCESS_TAG, new OARProcessHandler(
				proActiveDescriptor));
		this.addHandler(OARGRID_PROCESS_TAG, new OARGRIDProcessHandler(
				proActiveDescriptor));
		...
		this.addHandler(UNICORE_PROCESS_TAG, new UnicoreProcessHandler(
				proActiveDescriptor));
	}
	...
	public class ProcessHandler extends AbstractUnmarshallerDecorator implements
		...
		public void startContextElement(String name, Attributes attributes)
			...
			try {
				targetProcess = proActiveDescriptor
						.createProcess(id, className);
		...
		protected void notifyEndActiveHandler(String name,
			...
			if (name.equals(ENVIRONMENT_TAG)) {
				targetProcess.setEnvironment((String[]) activeHandler
						.getResultObject());
				...
				targetProcess.setCommandPath((String) activeHandler
						.getResultObject());
			}
		}
	...
	public class ProcessListHandler extends ProcessHandler implements
			ProActiveDescriptorConstants {
		...
		public void startContextElement(String name, Attributes attributes)
			...
			if (checkNonEmpty(fixedName) && checkNonEmpty(list)
				...
				((AbstractListProcessDecorator) targetProcess).setHostConfig(
						fixedName, list, domain, padding);
			}
			if (checkNonEmpty(hostlist)) {
				((AbstractListProcessDecorator) targetProcess).setHostList(
						hostlist, domain);
			}
			if (checkNonEmpty(closeStream) && closeStream.equals("yes")) {
				targetProcess.closeStream();
		...
		protected class PrunOptionHandler extends PassiveCompositeUnmarshaller {
			...
			protected void notifyEndActiveHandler(String name,
				...
				if (name.equals(HOST_LIST_TAG)) {
					prunSubProcess.setHostList((String) activeHandler
							.getResultObject());
				} else if (name.equals(HOSTS_NUMBER_TAG)) {
					prunSubProcess.setHostsNumber((String) activeHandler
							.getResultObject());
				} else if (name.equals(PROCESSOR_PER_NODE_TAG)) {
					prunSubProcess
							.setProcessorPerNodeNumber((String) activeHandler
									.getResultObject());
					...
					prunSubProcess.setBookingDuration((String) activeHandler
							.getResultObject());
				} else if (name.equals(PRUN_OUTPUT_FILE)) {
					prunSubProcess.setOutputFile((String) activeHandler
							.getResultObject());
				} else {
	...
	protected class PBSProcessHandler extends ProcessHandler {
		...
		protected class PBSOptionHandler extends PassiveCompositeUnmarshaller {
			...
			protected void notifyEndActiveHandler(String name,
				...
				if (name.equals(HOST_LIST_TAG)) {
					pbsSubProcess.setHostList((String) activeHandler
							.getResultObject());
				} else if (name.equals(HOSTS_NUMBER_TAG)) {
					pbsSubProcess.setHostsNumber((String) activeHandler
							.getResultObject());
				} else if (name.equals(PROCESSOR_PER_NODE_TAG)) {
					pbsSubProcess
							.setProcessorPerNodeNumber((String) activeHandler
					...
					pbsSubProcess.setScriptLocation((String) activeHandler
							.getResultObject());
				} else if (name.equals(BOOKING_DURATION_TAG)) {
					pbsSubProcess.setBookingDuration((String) activeHandler
							.getResultObject());
				} else if (name.equals(PRUN_OUTPUT_FILE)) {
					pbsSubProcess.setOutputFile((String) activeHandler
							.getResultObject());
				} else {
					super.notifyEndActiveHandler(name, activeHandler);
				}
	...
	protected class GridEngineProcessHandler extends ProcessHandler {
		...
		protected class GridEngineOptionHandler extends
			...
			protected void notifyEndActiveHandler(String name,
				...
				if (name.equals(HOST_LIST_TAG)) {
					geSubProcess.setHostList((String) activeHandler
							.getResultObject());
				} else if (name.equals(HOSTS_NUMBER_TAG)) {
					geSubProcess.setHostsNumber((String) activeHandler
							.getResultObject());
				} else if (name.equals(PARALLEL_ENVIRONMENT_TAG)) {
					geSubProcess.setParallelEnvironment((String) activeHandler
							.getResultObject());
				} else if (name.equals(SCRIPT_PATH_TAG)) {
					geSubProcess.setScriptLocation((String) activeHandler
							.getResultObject());
				} else if (name.equals(BOOKING_DURATION_TAG)) {
					geSubProcess.setBookingDuration((String) activeHandler
							.getResultObject());
				} else {
					super.notifyEndActiveHandler(name, activeHandler);
				}
	...
	protected class OARProcessHandler extends ProcessHandler {
		...
		public void startContextElement(String name, Attributes attributes)
			...
			if (checkNonEmpty(accessProtocol)) {
				((OARSubProcess) targetProcess)
						.setAccessProtocol(accessProtocol);
			}
		}
		...
		protected class OAROptionHandler extends PassiveCompositeUnmarshaller {
			...
			protected void notifyEndActiveHandler(String name,
				...
				if (name.equals(OAR_RESOURCE_TAG)) {
					oarSubProcess.setResources((String) activeHandler
							.getResultObject());
				}
				//Not yet supported
				...
				//                }
					...
					oarSubProcess.setScriptLocation((String) activeHandler
							.getResultObject());
				} else {
					super.notifyEndActiveHandler(name, activeHandler);
				}
	...
	protected class OARGRIDProcessHandler extends ProcessHandler {
		...
		public void startContextElement(String name, Attributes attributes)
			...
			String accessProtocol = (attributes.getValue("bookedNodesAccess"));
			if (checkNonEmpty(accessProtocol)) {
				((OARGRIDSubProcess) targetProcess)
						.setAccessProtocol(accessProtocol);
			}
		}

		protected class OARGRIDOptionHandler extends
				PassiveCompositeUnmarshaller {
			...
			protected void notifyEndActiveHandler(String name,
				...
				if (name.equals(OAR_RESOURCE_TAG)) {
					oarGridSubProcess.setResources((String) activeHandler
							.getResultObject());
				} else if (name.equals(OARGRID_WALLTIME_TAG)) {
					oarGridSubProcess.setWallTime((String) activeHandler
							.getResultObject());
				} else if (name.equals(SCRIPT_PATH_TAG)) {
					oarGridSubProcess.setScriptLocation((String) activeHandler
							.getResultObject());
				} else {
					super.notifyEndActiveHandler(name, activeHandler);
				}
	...
	protected class JVMProcessHandler extends ProcessHandler {
		...
		protected void notifyEndActiveHandler(String name,
			...
			if (name.equals(CLASSPATH_TAG)) {
				...
				jvmProcess.setPolicyFile(((String) activeHandler
						.getResultObject()).trim());
			} else if (name.equals(LOG4J_FILE_TAG)) {
				jvmProcess.setLog4jFile(((String) activeHandler
						.getResultObject()).trim());
				...
				jvmProcess.setClassname((String) activeHandler
						.getResultObject());
			} else if (name.equals(PARAMETERS_TAG)) {
				jvmProcess.setParameters((String) activeHandler
						.getResultObject());
			} // else if (name.equals(JVMPARAMETERS_TAG)) {
	...
	protected class MapRshProcessHandler extends ProcessHandler {
		...
		protected void notifyEndActiveHandler(String name,
			...
			if (name.equals(SCRIPT_PATH_TAG)) {
				((MapRshProcess) targetProcess)
						.setScriptLocation((String) activeHandler
								.getResultObject());
			} else {
				super.notifyEndActiveHandler(name, activeHandler);
			}
	...
	protected class BSubProcessHandler extends ProcessHandler {
		...
		protected class BsubOptionHandler extends PassiveCompositeUnmarshaller {
			...
			protected void notifyEndActiveHandler(String name,
				...
				if (name.equals(HOST_LIST_TAG)) {
					bSubProcess.setHostList((String) activeHandler
							.getResultObject());
				} else if (name.equals(PROCESSOR_TAG)) {
					bSubProcess.setProcessorNumber((String) activeHandler
							.getResultObject());
				} else if (name.equals(RES_REQ_TAG)) {
					bSubProcess.setRes_requirement((String) activeHandler
							.getResultObject());
				} else if (name.equals(SCRIPT_PATH_TAG)) {
					bSubProcess.setScriptLocation((String) activeHandler
							.getResultObject());
				} else {
					super.notifyEndActiveHandler(name, activeHandler);
				}
	...
	protected class GlobusProcessHandler extends ProcessHandler {
		...
		protected class GlobusOptionHandler extends
				PassiveCompositeUnmarshaller {
			public GlobusOptionHandler() {
				this
						.addHandler(GLOBUS_COUNT_TAG,
								new SingleValueUnmarshaller());
			}
			...
			protected void notifyEndActiveHandler(String name,
				...
				if (name.equals(GLOBUS_COUNT_TAG)) {
					globusProcess.setCount((String) activeHandler
							.getResultObject());
				} else {
					super.notifyEndActiveHandler(name, activeHandler);
				}
