 ...
 * @since ProActive 2.2
 */
public class GlobalFTServer extends UnicastRemoteObject
    implements CheckpointServer, RecoveryProcess, LocationServer, FaultDetector,
        ResourceServer {
    ...
    public synchronized int storeCheckpoint(Checkpoint c, int inc) {
        if (inc < this.globalIncarnation) {
            logger.warn("Object with incarnation " + inc +
        ...
        if (this.checkLastGlobalState()) {
            try {
                this.broadcastFTEvent(new GlobalStateCompletion(
    ...
    public synchronized void addInfoToCheckpoint(CheckpointInfo ci,
        UniqueID id, int sequenceNumber, int inc) throws RemoteException {
        if (inc < this.globalIncarnation) {
            logger.warn("Object with incarnation " + inc +
                " is trying to store checkpoint infos");
            return;
        }
        ...
        if (c.getCheckpointInfo() != null) {
            logger.error("[CKPT] **ERROR** INFOS already set for checkpoint " +
                sequenceNumber);
        }
        c.setCheckpointInfo(ci);
        logger.info("[CKPT] INFOS have been added to checkpoint " +
            sequenceNumber);
        if (displayCkptSize) {
            logger.info("[CKPT] Size of infos for ckpt " + sequenceNumber +
                " : " + this.getSize(ci) + " bytes");
            logger.info("[CKPT] Total size of ckpt " + sequenceNumber + " : " +
                this.getSize(c) + " bytes");
        }

        // updating monitoring
        int index = c.getIndex();
        if (index > this.lastUpdatedCkpt) {
            this.lastUpdatedCkpt = index;
        }
    ...
    public synchronized UniversalBody searchObject(UniqueID id,
        UniversalBody oldLocation, UniqueID caller) throws RemoteException {
    ...
    public synchronized void updateLocation(UniqueID id,
        UniversalBody newLocation) throws RemoteException {
        UniversalBody currentLocation = (UniversalBody) (this.locations.get(id));
        if (currentLocation == null) {
            ...
            if (currentLocation.equals(newLocation)) {
                logger.info("[LOCATION] location of " + id + " is already " +
                    newLocation.getNodeURL());
            } else {
                logger.info("[LOCATION] " + id +
                    " is updating its location : " + newLocation.getNodeURL());
                this.locations.put(id, newLocation);
            }
        }
    }
        ...
        try {
            ...
            while (itBodies.hasMoreElements()) {
                ...
                if (current.equals(failed)) {
                    ...
                    Thread t = new RecoveryThread(toSend,
                    ...
                    try {
                        isDead = this.isUnreachable(toRecover);
                    } catch (Exception e) {
                    }
                    if (isDead) {
                        Node node = this.getFreeNode();
    ...
    private class RecoveryThread extends Thread {
        ...
        public RecoveryThread(Checkpoint toSend, int inc, Node receiver) {
            ...
            this.setName("RecoveryThread for " +
                this.receiver.getNodeInformation().getURL());
        }

        public void run() {
            try {
                logger.info("[RECOVERY] " + this.toSend.getBodyID() +
                    " is recovering from " + toSend.getIndex() + " ...");
    ...
    public synchronized void failureDetected(UniqueID id)
        throws RemoteException {
        // id is recovering ??
    ...
    public synchronized void updateState(UniqueID id, int state)
        throws RemoteException {
    ...
    private class FaultDetectorThread extends Thread {
        ...
        public FaultDetectorThread(LocationServer ls, RecoveryProcess rp,
            FaultDetector fd) {
            this.loc = ls;
            this.rec = rp;
            this.fdet = fd;
            ...
            while (true) {
                try {
                    synchronized (GlobalFTServer.this) {
                        ...
                        GlobalFTServer.logger.info("[FAULT DETECTOR] Scanning " +
                            al.size() + " objects ...");
                        while (it.hasNext()) {
    ...
    private void printDebug(String location) {
        ...
        System.out.println("   * number of registred bodies : " +
            this.bodies.size());
        System.out.println("   * latest registred checkpoint index : " +
            this.lastRegisteredCkpt);
        System.out.println("   * latest global state : " +
            this.lastGlobalState);
    }
