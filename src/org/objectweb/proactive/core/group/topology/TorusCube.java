...
public class TorusCube extends Torus { // implements Topology3D {
	 ...
	 */
	public TorusCube (Group g, int height, int width, int depth) throws ConstructionOfReifiedObjectFailedException {
	...
	public Torus TorusX (Object o) {
		...
		for (int i = 0 ; i < this.getHeight() ; i++) {
			for (int j = 0 ; j < this.getDepth() ; j++) {
				tmp.add(this.get((begining+(i*this.getWidth()))+(j*(this.getWidth()*this.getHeight()))));
			}
		}
		Torus result = null;
		try {
