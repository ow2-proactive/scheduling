...
public class HierarchicalBarRenderer extends BarRenderer {
    ...
    public void drawItem(Graphics2D g2, CategoryItemRendererState state,
        ...
        // BAR X
        double rectX = domainAxis.getCategoryMiddle(column, this
        ...
        if (row == 0) {
            ...
            while (it.hasNext()) {
                try {
                    ...
                    // "+dataValue);
                    // BAR X
                    rectX = domainAxis.getCategoryMiddle(column, this
                    ...
                    rectWidth = widthFactor * state.getBarWidth();
                    rectX = rectX + (1 - widthFactor) * state.getBarWidth()
                            / 2.0;
                    if (seriesCount > 1) {
                        // needs to be improved !!!
                        shift = rectWidth * 0.20 / (seriesCount - 1);
                    ...
                    if (numElement == 0) {
                        ...
                        double pW = Double.valueOf(parent
                                .getAttributeValue("dev"));
                        double pX = Double.valueOf(parent
                                .getAttributeValue("min"));
                        int numChild = Integer.valueOf(parent
                    ...
                    double childSum = Double.valueOf(elt
                    ...
                    if (childSum < rrH && childSum > 0 && childSum / rrH < 0.95) {
                        childSumLine = new Rectangle2D.Double(rrX, rectY, rrW,
                                1);
                    }
                    Paint itemPaint = this.getItemPaintFromName(name,
                            this.series, column);
                    GradientPaintTransformer t = this
        ...
        if (state.getInfo() != null) {
            ...
            if (entities != null) {
                String tip = null;
                CategoryToolTipGenerator tipster = this.getToolTipGenerator(
                        row, column);
                if (tipster != null) {
