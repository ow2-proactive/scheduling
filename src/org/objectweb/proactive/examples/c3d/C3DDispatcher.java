...
public class C3DDispatcher implements org.objectweb.proactive.RunActive {
  ...
  /*
   * Stores the useres in a hash table
   */
  ...
  public void init() {
    try {
      ...
      for (int n = 0; n < engines; n++) {
        ...
        C3DRenderingEngine tmp = (C3DRenderingEngine)org.objectweb.proactive.ProActive.newActive("org.objectweb.proactive.examples.c3d.C3DRenderingEngine", param, node);
        log("New rendering engine " + n + " created at " + node);
        if (n == 0)
          li_enginesUsed.add(node);
        else li_enginesAvailable.add(node);

        // adds the engine in the hashtable
        h_engines.put(node, tmp);
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public void setPixels(int[] newpix, Interval interval, int engine_number) {
    ...
    if (!int_stack.empty()) {
      ...
      if (previousTime != -1)
        showMessageAll("Speed Up : " + java.lang.Math.rint(((double)previousTime / elapsed) * 1000) / 1000 + "\n");
      else
        showMessageAll("");
  ...
  public void changeColorAll() {
    ...
    for (Enumeration e = h_users.elements(); e.hasMoreElements();)
      ((User)e.nextElement()).getObject().getUserFrame().getB_left().setBackground(Color.yellow);
  }
 ...
 private void registerDispatcher(String nodeURL) { 
   ...
   try {
     org.objectweb.proactive.core.node.Node node = org.objectweb.proactive.core.node.NodeFactory.getNode(nodeURL);
     s_access_url = "//" + node.getNodeInformation().getInetAddress().getHostName() + "/" + C3D_DISPATCHER_BIND_NAME;
     org.objectweb.proactive.ProActive.register(org.objectweb.proactive.ProActive.getStubOnThis(), s_access_url);
  ...
  public void showDialog(int i_user, String subject, String s_message) {
    ((User)h_users.get(new Integer(i_user))).getObject().dialogMessage(subject, s_message);
  }
  ...
  public void addSphere(Sphere s) {
    ...
    if (!int_stack.isEmpty()) {
      ...
      showMessageAll("A Sphere has been added\nX:" + Math.floor(s.getCenter().x) + " Y:" + Math.floor(s.getCenter().y) + " Z:" + Math.floor(s.getCenter().y));
      render();
  ...
  class C3DDispatcherFrame extends Frame implements ActionListener {
    ...
    public C3DDispatcherFrame() {
      ...
      Label l_machine = new Label("on machine: " + s_localhost + " (" + System.getProperty("os.name") + " " + System.getProperty("os.version") + ")");
    ...
    private void exit() {
      try {
        org.objectweb.proactive.ProActive.unregister(C3D_DISPATCHER_BIND_NAME);
      } catch (Exception e) {
...
class Election extends Thread {
  ...
  public Election(int i_user, Integer wish, C3DDispatcher c3ddispatcher) {
    ...
    c3ddispatcher.showMessageExcept(i_user, "   User " + c3ddispatcher.nameOfUser(i_user) + " wants to rotate " + voteString(wish));
  ...
  public synchronized void run() {
    ...
    for (Enumeration e = wishes.keys(); e.hasMoreElements();) {
      ...
      c3ddispatcher.showMessageAll("   User " + c3ddispatcher.nameOfUser(i_user) + " voted '" + voteString(wish) + "'");
