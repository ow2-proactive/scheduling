...
public class Domain implements Serializable {
     ...
     */
    public void init(Domain[] domainArray, Displayer dp, QuadTree tree,
        int maxIter) {
        this.display = dp;
    ...
    public void addNeighbour(int domainIdent, int checkIter) {
        if ((checkIter - 1) > iter) { // checkIter - iter = 1 is ok, means start sending on next iteration 
            killsupport.abort(new RuntimeException("Domain " + domainIdent +
                    "[" + checkIter + "] asking for a future info to Domain" +
                    identification + "[" + iter + "] , not possible!"));
        }
    ...
    public void removeNeighbour(int domainIdent) {
        ...
        if (!correct) {
            killsupport.abort(new NoSuchElementException("Domain " +
                    domainIdent +
                    " cannot be removed, because it's not in neighbour Group! "));
        }
    }
    ...
    public void setValue(Info inf, int receivedIter) { // FIXME : this never gets called if nb Domains=1
        ...
        if (this.iter == receivedIter) {
            ...
            if (this.nbReceived > this.nbNeighbours) { // This is a bad sign!
                killsupport.abort(new NullPointerException("Domain " +
                        identification + " received too many answers"));
            }
            if (this.isLeaf) {
                addToTotalForce(inf);
            } else if (isSon(inf.identification)) {
                this.info.addSon(inf);
            } else {
                killsupport.abort(new NullPointerException("Domain " +
                        identification + " received not son!!"));
            }
            ...
            if (this.iter > receivedIter) {
                killsupport.abort(new NullPointerException(
    ...
    private void addToTotalForce(Info distantInfo) {
        ...
        try {
            ...
            if (distantTreeNode.Q == null) { // there are no subTrees, compute with every planet  
                ...
                this.nbNeighbours--;
                this.nbReceived--; // totally hide the existence of this Domain 
    ...
    public void sendValueToNeighbours() {
        ...
        if (this.iter < this.maxIter) {
            ...
            } else if (this.isLeaf) { // only leaves contain enough information, concerning planets
                for (int i = 0; i < this.info.planets.length; i++) {
                    Planet planet = this.info.planets[i];
                    display.drawBody((int) planet.x, (int) planet.y,
                        (int) planet.vx, (int) planet.vy, (int) planet.mass,
                        (int) planet.diameter, planet.identification, hostName);
                }
            }
            treatPremature();
        } else if (this.identification == 0) { // only need one quit signal man, and 0 always has smallest iteration!
            killsupport.quit();
        }
    }
