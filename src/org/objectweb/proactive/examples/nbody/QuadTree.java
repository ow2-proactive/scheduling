...
/**
 * TODO Z7 now: fixed is nb bodies per domain, variable is nb QuadTrees . next : nb QuadTrees fixed (ie AO), nb bodies variable
 * @author irosenbe
 * @see also http://www.reed.edu/~jimfix/MSRI/quadtree/Quadtree.java
 */
public class QuadTree implements Serializable {
    
    private final int MAX_BODIES_IN_DOMAIN = 4;
     ...
     * 
     */
    public void myConstructor(Rectangle R, Vector bodies) {
        info = new Info(bodies,R);
        
        if (info.numPlanets >  MAX_BODIES_IN_DOMAIN)	{ 	    // Split into sub-trees
            
            Point2D middle = average(bodies, info.numPlanets);
            ...
            //            Q=(QuadTree[]) trees.toArray(); TODO Z9 why doesn't this work?
     ...
     */
    private Point2D average(Vector bodies, int numPlanets) {
        // 	TODO Z6 remove this assertion
        assert bodies.size() == numPlanets : "param2 = param1.size()";
        // end assertion
    ...
    //TODO Z5 write method when bodies may change Domain 
 ...
 */
    ...
    /**
     * Removes all leaves from tree, ie all nodes that do not have sons are removed.
     * @param treeNode
     */
/*
    public void setAllLeavesToNullExcept(QuadTree treeNode) {
        // FIXME : does this method make sense ? USEFULNESS?
        return ;
                Vector nodes = new Vector();
                QuadTree t = this;
                while ( !nodes.isEmpty() ){
                    t = (QuadTree) nodes.remove(0);
                    if (t.Q == null && t != treeNode) {
                        t = null;
                    }
                    else {
                        for (int i = 0 ; i < t.Q.length ; i++ )
  ...
  */  
     ...
     */
    public void setNodeInfo(Info inf, int id) {
        QuadTree t = getNode (id);
        if (id == label) {
            
        }
        double oldx= t.info.x;
        int oldnbplanets= t.info.getPlanets().size();
        t.info = inf;
    }
    ...
    public void recomputeCenterOfMass() {
        ...
        if (Q==null) {  // If no other nodes, make the array the list of planets
            Vector dummy = info.getPlanets(); // TODO Z9 : why doesn't toArray() work?
