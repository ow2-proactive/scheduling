...
public class DinnerLayout implements org.objectweb.proactive.Active {
  ...
  public ObjectForSynchronousCall init() {
    ...
    // Creates the Table 
    try {
      manager = (Table)org.objectweb.proactive.ProActive.newActive(Table.class.getName(), params, url);
    } catch (Exception e) {
      e.printStackTrace();
    }
    ...
    for (int n = 0; n < 5; n++) {
      ...
      try {
        phils[n] = (Philosopher)org.objectweb.proactive.ProActive.newActive(Philosopher.class.getName(), params);
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
   ...
   */
  protected class UserFrame extends javax.swing.JPanel implements java.awt.event.ActionListener, java.awt.event.MouseListener {
