...
public class SessionInitializer {
	...
	public boolean authenticate_mutual(SessionsManagerInt sessionsManager) throws AuthenticationException {
		...
		String B2 =
				emitterCertificate.get_CertificateIdentity().getDomainName();
		...
		boolean certificateValidity =
				certificateChecker.checkValidity(emitterCertificate,
		...
		boolean signatureValidity =
				signingEngine.checkSignature(return_ticket.signedAuthenticationTicketProperty,
	...
	public boolean authenticate_unilateral(SessionsManagerInt sessionsManager) throws AuthenticationException {
		...
		try {
			authenticationTicket =
					sessionsManager.authenticate_unilateral(sessionID, rb, B);
		}
		catch (AuthenticationException e) {
			System.out.println("SessionInitializer : AuthenticationException : "
					 + e);
		}
		catch (java.rmi.RemoteException e) {
			System.out.println("SessionInitializer :Remote Exception : " + e);
		...
		boolean certificateValidity =
				certificateChecker.checkValidity(emitterCertificate, /* emitterCertificate.get_acPublicKey() */
				acPublicKey);
		...
		AuthenticationTicketProperty properties =
				new AuthenticationTicketProperty();
		...
		try {
			...
			System.out.println("SessionInitializer : Exception in AuthenticationTicketProperty extraction : "
					 + e);
		}
	...
	public boolean negociate_key(SessionsManagerInt sessionsManager) throws KeyExchangeException {
		...
		Object encryptedSessionKeyPart1 = encryptionEngine.asymmetric_encrypt(part1,
				otherPublicCertificate.get_certificatePublicKey());
		...
		boolean signatureValidity =
				signingEngine.checkSignature(return_ticket.signedConfidentialityTicketProperty,
				otherPublicCertificate.get_certificatePublicKey());
		...
		try {
			properties2 =
					(ConfidentialityTicketProperty) ((SignedObject) return_ticket.signedConfidentialityTicketProperty).getObject();
		}
		catch (Exception e) {
			System.out.println("Exception in AuthenticationTicketProperty extraction : "
					 + e);
		}
		...
		part1 =
				(byte[]) encryptionEngine.asymmetric_decrypt(return_ticket.encryptedSessionKeyPart1,
				privateKey);
		part2 =
				(byte[]) encryptionEngine.asymmetric_decrypt(return_ticket.encryptedSessionKeyPart2,
				privateKey);
		...
		try {
			...
			System.out.println("SessionInitializer : Exception 2 in sessionKey reconstitution :"
					 + e);
		}
