  ...
  private void fireSpyEventInternal(SpyEvent spyEvent) {
    switch (spyEvent.getType()) {    
      case SpyEvent.BODY_CREATION_EVENT_TYPE:
        ...
        spyEventListener.activeObjectAdded(spyEvent.getBodyID(), event.getNodeURL(), event.getClassName(), event.isActive());
        break;
       
      case SpyEvent.BODY_EVENT_TYPE:
        BodySpyEvent bse = (BodySpyEvent) spyEvent;
        spyEventListener.activeObjectChanged(spyEvent.getBodyID(), bse.isActive(), bse.isAlive());
        break;
      ...
      case SpyEvent.REQUEST_RECEIVED_MESSAGE_TYPE:
        spyEventListener.requestMessageReceived(spyEvent.getBodyID(), spyEvent);
      break;
