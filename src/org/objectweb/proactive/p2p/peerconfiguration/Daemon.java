...
import org.objectweb.proactive.p2p.registry.P2PRegistry;
...
public class Daemon implements Runnable {
    ...
    private P2PRegistry register = null;
    ...
    public Daemon(String file) {
        ...
        try {
            register = (P2PRegistryImpl) ProActive.lookupActive(P2PRegistryImpl.class.getName(),
                    p2pRegistry);
    ...
    public static void main(String[] args) {
        if (args.length < 1) {
            System.err.println(
                "[Daemon ERROR] use: daemon    file_schedule.xml");
            System.exit(-1);
        }
    ...
    private synchronized void work() {
        ...
        for (i = 0; i < schedule.length; i++) {
            if (schedule[i].WorkToday()) {
                schedToday.add(schedule[i]);
            }
        }
        ...
        for (i = 0; i < schedOrdered.length; i++) {
            int now = (today.get(Calendar.HOUR_OF_DAY) * 100) +
            ...
            if (schedNow.getStartTime() > now) {
                ...
                try {
                    ...
                    now = (today.get(Calendar.HOUR_OF_DAY) * 100) +
                        today.get(Calendar.MINUTE);
                }
            }

            // the right schedule?
            if ((schedNow.getStartTime() + schedNow.getWorkTime()) < now) {
                continue;
            }

            // start it!
            if (!isAlive) {
                startIt(schedNow.getMaxLoad());
            }

            // If the schedule < 24 hours, stop it
            if ((schedNow.getWorkTime() < 2400) && isAlive) {
                wakeUp = ((schedNow.getStartTime() + schedNow.getWorkTime()) -
                    now) * 60 * 1000;
                try {
                    wait(wakeUp);
    ...
    private synchronized void startIt(double maxload) {
        ...
        try {
            ...
            System.err.println(
                "[Daemon ERROR] Cannot get the ProActive Runtime");
        }
    }
    ...
    private synchronized void killIt() {
        ...
        isAlive = false;
        try {
            register.unregister(runtime.getURL());
        } catch (ProActiveException e) {
            System.err.println(
                "[Daemon ERROR] Cannot get the ProActive Runtime");
        }
