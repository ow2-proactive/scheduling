...
public class PriorityQueueImpl implements TaskQueue {
     ...
     */
    public Task next() {
        Task task = (Task) this.queue.last();
    ...
    public void informNewBestResult(Result newBest, String taskTag) {
        ...
        while (it.hasNext()) {
            ...
            if ((current.getTag() != null) &&
                    (current.getTag().compareTo(taskTag) == 0)) {
                current.incPriority();
                last = current;
            }
        }
        if (last != null) {
            this.queue = new TreeSet((Collection) this.queue);
            logger.info("The tasks of " + last.getTag() +
                " have a new priority " + last.getPriority());
        } else {
            logger.info("NO TASKS OF " + taskTag);
        }
    }
}
