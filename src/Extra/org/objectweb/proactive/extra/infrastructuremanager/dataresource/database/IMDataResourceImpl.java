...
public class IMDataResourceImpl implements IMDataResource, Serializable {
	...
	public int getSizeListFreeIMNode() { return listFreeIMNode.size(); }
	public int getSizeListBusyIMNode() { return listBusyIMNode.size(); }
	public int getSizeListPad()        { return listPad.size();        }
	...
	public void removeNode(String padName, String vnName) {
		for(IMNode imnode : listBusyIMNode) {
			if( imnode.getPADName().equals(padName) & imnode.getVNodeName().equals(vnName)) {
				listBusyIMNode.remove(imnode);
			}
		}
		for(IMNode imnode : listBusyIMNode) {
	...
	public Node[] getAtLeastNNodes(int nb) throws NodeException {
		...
		if (logger.isInfoEnabled()) {
			logger.info("getAtLeastNNodes, user want " + nb + " nodes and there is " + nbFreeIMNode + " free node(s)");
		}
	...
	public Node getNode() throws NodeException {
		int nbFreeIMNode     = listFreeIMNode.size();
		if (logger.isInfoEnabled()) {
			logger.info("getNode, user want a node, and there is " + nbFreeIMNode + " free node(s)");
		}
		if( nbFreeIMNode != 0 ) {
