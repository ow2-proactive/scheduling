...
import org.objectweb.proactive.extra.scheduler.policy.GenericPolicy;
...
public class Scheduler implements RunActive, RequestFilter {
    ...
    public Scheduler(String p, GenericResourceManager rm, String policyFactory)
        ...
        try {
            ...
            policy = (GenericPolicy) Class.forName(p)
                                          .getMethod(policyFactory,
                    GenericResourceManager.class).invoke(null, rm);
        } catch (java.lang.ClassNotFoundException e) {
            logger.error("The policy class couldn't be found " + e.toString());
            throw e;
        } catch (java.lang.IllegalAccessException e) {
            logger.error("The method cannot be acessed " + e.toString());
            throw e;
            ...
            logger.error("The argument passed isnt correct" + e.toString());
            throw e;
        } catch (InvocationTargetException e) {
            logger.error("policy constructor threw an exception" +
                e.toString());
            throw e;
    ...
    public void configureScheduler() throws Exception {
        try {
            SCHEDULER_TIMEOUT = Long.parseLong(System.getProperty(
                        "extra.scheduler.scheduler_timeout"));

            TIME_BEFORE_TEST_ALIVE = Long.parseLong(System.getProperty(
                        "extra.scheduler.time_before_test_alive"));

            if (this.SCHEDULER_TIMEOUT < 1) {
                throw new Exception(
                    "SCHEDULER_TIMEOUT must be a positive integer");
            }
            if (this.TIME_BEFORE_TEST_ALIVE < 1) {
                throw new Exception(
                    "TIME_BEFORE_TEST_ALIVE must be a positive integer");
            }
        } catch (Exception e) {
            logger.error("An error trying to parse scheduler arguments " +
                e.toString());
            throw e;
        }
    }
    ...
    public void runActivity(Body body) {
        ...
        if (logger.isDebugEnabled()) {
            ...
            if (service.getOldest() != null) {
                logger.debug("oldest method is " +
                    service.getOldest().getMethodName());
            } else {
                logger.debug("terminate not called yet!!");
            }
        ...
        do {
            try {
                Thread.sleep(SCHEDULER_TIMEOUT);
            } catch (Exception e) {
            }
            ...
            onlyTerminateLeft = ((service.getRequestCount() == 1) &&
                service.hasRequestToServe("terminateScheduler")) ||
                (service.getRequestCount() == 0);
            if (logger.isDebugEnabled() && (service.getRequestCount() > 0)) {
                logger.debug(
                    "Warning: must check for the case where more than one terminate was submitted" +
                    (i++) + " " + runningTasks.size() + " " +
    ...
    private void handlePolicyNFinishedTasks() {
        if (hardShutdown) {
            ...
            logger.warn(
                "this is a soft shutdown, what do u want to do with the queue and finished tasks??");
        }
    }
    ...
    */
    ...
    public GenericTypeWrapper<Info> info(String taskID) {
        ...
        //if it couldnt be found
            ...
            return new GenericTypeWrapper<Info>(new Info(Status.NEW, taskID,
                    "unknown", "unknown", -1, -1, -1, -1, -1));
        }
    }
    ...
    public InternalResult getResult(String taskID, String userName) {
        ...
        for (int i = 0; i < finishedTasks.size(); i++)
            if (finishedTasks.get(i).getTaskID().equals(taskID) &&
                    finishedTasks.get(i).getUserName().equals(userName)) {
    ...
    public boolean acceptRequest(Request request) {
        ...
        if (request.getMethodName() == "getResult") {
            ...
            else if ((task != null) && (task.status == Status.QUEUED) &&
                    shutdown) {
                createError(taskID, userName,
                    "Shcheduler is shutting down, task wont be scheduled");
                return true;
            } else if ((task != null) && (task.status == Status.FAILED) &&
                    shutdown) {
                createError(taskID, userName,
                    "Due to an internal error, task execution failed. However, sheduler is shutting down.task wont be scheduled");
                return true;
            }
    ...
    public void killAllRunning() {
        ...
        while (!runningTasks.isEmpty()) {
            if (ProActive.isAwaited(runningTasks.get(0).result)) {
                ...
                toBeKilled.result.setErrorMessage(
                    "Task Was killed before execution finished.");
                ...
                //it will always throw an exception because the kill methid kills the jvm, so it is actually normal
                    ...
                    logger.info("Task " + toBeKilled.getTaskID() +
                        "was killed");
                } catch (Exception e) {
                    e.printStackTrace();
                    logger.info("Something went wrong killing Task " +
                        toBeKilled.getTaskID());
                }
    ...
    public void shutdown(BooleanWrapper immediate) {
        if (shutdown == true) {
            return; // shutdown already intiated
        }
    ...
    private void pingAll() {
        if (!runningTasks.isEmpty() &&
                ((System.currentTimeMillis() - previousRunOfPingAll) > TIME_BEFORE_TEST_ALIVE)) {
            ...
            while (i < runningTasks.size()) {
                try {
                    ...
                    logger.error("Task " + failed.getTaskID() +
                        " has failed and will be returned to policy for rescheduling");
                }
            }
            ...
            if (!troubledNodes.isEmpty()) {
                ...
                freeDeadNodes(troubledNodes);
                if (logger.isDebugEnabled()) {
                    logger.warn(troubledNodes.size() +
                        " nodes are trobled and were returned to resource manager.");
                }
            }
    ...
    public BooleanWrapper del(String taskID, String userName) {
        ...
        if (isInTheQueue(taskID).booleanValue()) {
            ...
            toBeKilled.result = new InternalResult();
            toBeKilled.result.setErrorMessage(
                "Task has been deleted from the scheduler queue");
            finishedTasks.add(toBeKilled);
