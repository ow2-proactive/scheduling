...
public class Test extends ComponentTest {
    ...
    private Message result1, result2, result3, result4, result5, result6;
    private final String expectedResult = "foo-->a-->b-->a";
    
    private Component systemWithWrappingWithShortcuts, systemWithWrappingWithoutShortcuts, systemWithoutWrapping ;
    ...
    public void action() throws Exception {
        ...
        result2 = ((I1) systemWithWrappingWithoutShortcuts.getFcInterface("i1")).processInputMessage(new Message("foo"));
        ...
        result3 = ((I1) systemWithWrappingWithShortcuts.getFcInterface("i1")).processInputMessage(new Message("foo"));
        ...
        result4 = ((I1) systemWithWrappingWithShortcuts.getFcInterface("i1")).processInputMessage(new Message("foo"));
...
//        result6 = ((I1) systemWithWrappingWithShortcuts.getFcInterface("i1")).processInputMessage(new Message("foo"));
    ...
    private void initializeComponentSystems() throws Exception {
        ...
        Fractal.getBindingController(wrappedAWithoutShortcuts).bindFc("i2", wrappedBWithoutShortcuts.getFcInterface("i2"));
        ...
        for (int i = 0; i < NB_WRAPPERS; i++) {
            wrappedAWithShortcuts = wrapWithSynchronousCompositeOfTypeA(NB_WRAPPERS - i, wrappedAWithShortcuts);
        }

        Component wrappedBWithShortcuts = Setup.createPrimitiveB1();
        for (int i = 0; i < NB_WRAPPERS; i++) {
            wrappedBWithShortcuts = wrapWithSynchronousCompositeOfTypeB(NB_WRAPPERS - i, wrappedBWithShortcuts);
        }
        
        Fractal.getBindingController(wrappedAWithShortcuts).bindFc("i2", wrappedBWithShortcuts.getFcInterface("i2"));
    ...
    private void resetComponentSystem() throws IllegalContentException, IllegalLifeCycleException, NoSuchInterfaceException, IllegalBindingException {
    ...
    public boolean postConditions() throws Exception {
        return (expectedResult.equals(((Message)ProActive.getFutureValue(result4)).getMessage()) && expectedResult.equals(((Message)ProActive.getFutureValue(result3)).getMessage()) && expectedResult.equals(((Message)ProActive.getFutureValue(result2)).getMessage()) && expectedResult.equals(ProActive.getFutureValue(((Message)ProActive.getFutureValue(result1)).getMessage())));
    }
}
