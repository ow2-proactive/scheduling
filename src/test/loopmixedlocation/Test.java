...
public class Test implements ActiveWithMixedLocation, Serializable {
    ...
    public Test(String[] nodes) {
     ...
     public void live(Body body) {
        try {
            while (body.isActive()) {
                System.out.println("Test pending requests = "
                                   + body.getRequestQueue().size());
    ...
    public static void main(String[] args) {
        ...
        try {
            t = (Test) ProActive.newActive(Test.class.getName(), arg);
            s = (Source) ProActive.newActive(Source.class.getName(), null);
        } catch (Exception e) {
            e.printStackTrace();
        }
        s.setTarget(t);
