...
public class RESTfulSchedLogClient extends BaseRESTfulSchedClient {
    ...
    public Arguments getDefaultParameters() {
        ...
        defaultParameters.addArgument(PARAM_LOG_CLIENT_REFRESH_TIME,
                "${logClientRefreshTime}");
    ...
    protected SampleResult doRunTest(JavaSamplerContext context)
        ...
        waitForNextCycle(getTimestamp(),
                context.getIntParameter(PARAM_LOG_CLIENT_REFRESH_TIME));
        return result;
    }

    private SampleResult fetchLiveLogs(String jobId) {
        String resourceUrl = (new StringBuilder(getConnection().getUrl()))
                .append("/jobs/").append(jobId).append("/livelog").toString();
        return getResource(getClientSession(),
                String.format("job('%s')'s livelog", jobId), resourceUrl);
    }
