...
public class RESTfulSchedStateClient extends BaseRESTfulSchedClient {
    ...
    public Arguments getDefaultParameters() {
        ...
        defaultParameters.addArgument(PARAM_STATE_CLIENT_REFRESH_TIME,
                "${stateClientRefreshTime}");
    ...
    protected SampleResult doRunTest(JavaSamplerContext context)
        ...
        if (!isEmpty(availableJobIds)) {
            ...
            if (quotient == 0) {
                ...
                for (int index = 0; index <= quotient; index++) {
                    result.addSubResult(fetchSchedulerState(index * pageSize,
                            pageSize));
                }
            }
        } else {
        ...
        waitForNextCycle(getTimestamp(),
                context.getIntParameter(PARAM_STATE_CLIENT_REFRESH_TIME));
        return result;
    }
