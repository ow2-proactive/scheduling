 ...
 * each.
 */
public abstract class BaseRESTfulRMClient extends BaseRESTfulClient implements
        JavaSamplerClient {
    ...
    public Arguments getDefaultParameters() {
        ...
        defaultParameters.addArgument(PROP_CLIENT_REFRESH_TIME,
                "${clientRefreshTime}");
    ...
    protected List<Host> getMonitoringHostSet() {
        String serialized = JMeterUtils
                .getProperty(clientSpecificHostPropKey());
        if (serialized != null) {
            try {
                @SuppressWarnings("unchecked")
                List<Host> hostSet = (List<Host>) getObjectMapper().readValue(
                        serialized.getBytes(), new TypeReference<List<Host>>() {
                        });
                return hostSet;
    ...
    protected List<Node> getMonitoringNodeSet() {
        String serialized = JMeterUtils
                .getProperty(clientSpecificNodePropKey());
        if (serialized != null) {
            try {
                @SuppressWarnings("unchecked")
                List<Node> nodeSet = (List<Node>) getObjectMapper().readValue(
                        serialized.getBytes(), new TypeReference<List<Node>>() {
                        });
                return nodeSet;
            } catch (IOException ioe) {
                logError("", ioe);
