...
public class ActiveObjectPingerThreadTest1 {
	...
	static class TestProxy extends ActiveObjectProxy<TestActiveObject> {
		...
		@Override
		protected boolean doPingActiveObject(
				TestActiveObject activeObject) {
		...
		public void teminate() {
			asyncCallActiveObject(
					new ActiveObjectAccess<TestActiveObject>() {
	...
	public void testSuccesfulPing() throws Exception {
		...
		ActiveObjectPingerThread thread = new ActiveObjectPingerThread(proxy,
				100,
				3000,
				listener);
		thread.start();
