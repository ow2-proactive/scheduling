...
public class SigarDatabaseEventsExposer {
    public void registerMBeans(MBeanServer mbs) throws MalformedObjectNameException, NotCompliantMBeanException,
        ...
        String dbTables = System.getProperty("sigar.databaseevents.tables");

        for (String table : dbTables.split(",")) {
            ObjectName name = new ObjectName("sigar:Type=DatabaseEvents,Name=" + table);
            SigarDatabaseEvents dbEvents = new SigarDatabaseEvents(dbUrl, dbUser, dbPassword, table);
            if (!mbs.isRegistered(name)) {
                mbs.registerMBean(dbEvents, name);
            }
        }
    }
}
