...
public final class ResourceManagerJMXTest extends RMFunctionalTest {
    ...
    public void action() throws Exception {
        ...
        final JMXServiceURL jmxRoServiceURL = new JMXServiceURL(
            auth.getJMXConnectorURL(JMXTransportProtocol.RO));
    ...
    private void jmxRemoteObjectAsAdmin(Credentials adminCreds, JMXServiceURL jmxRoServiceURL,
            ObjectName allAccountsMBeanName, ObjectName managementMBeanName) throws IOException,
    ...
    private void jmxURLsAreWellFormed(JMXServiceURL jmxRmiServiceURL, JMXServiceURL jmxRoServiceURL,
            String suffix) {
        ...
        assertTrue("The jmxRmiServiceURL URLPath must end with " + suffix, jmxRmiServiceURL.getURLPath()
                .endsWith(suffix));
        ...
        assertTrue("The jmxRoServiceURL URLPath must end with " + suffix, jmxRoServiceURL.getURLPath()
                .endsWith(suffix));
    ...
    private void jmxRMIAsUser(JMXServiceURL jmxRmiServiceURL, ObjectName allAccountsMBeanName,
        ...
        assertTrue("MyAccountMBean must be registered in the MBean server",
                conn.isRegistered(myAccountMBeanName));
        ...
        assertTrue("RuntimeDataMBean must be registered in the MBean server",
                conn.isRegistered(runtimeDataMBeanName));
        ...
        assertEquals("Incorrect value of " + attribute.getName() + " attribute", "STARTED",
                attribute.getValue());
        ...
        for (final MBeanAttributeInfo att : mBeanInfo.getAttributes()) {
            ...
            try {
                ...
                assertTrue(
                        "The attribute " + attName + " must not be accessible with user-level permissions",
    ...
    private void simultaneousRMIAndROConnections(Credentials adminCreds, JMXServiceURL jmxRmiServiceURL,
        ...
        assertFalse("In case of simultaneous RMI and RO JMX connections they must not be equal",
                conRmi.equals(conRo));

        assertFalse(
                "In case of simultaneous RMI and RO JMX connections the connectors must not provide the same "
                    + "connection ids",
                jmxRmiConnector.getConnectionId().equals(jmxRoConnector1.getConnectionId()));

        RMTHelper.log("Test JMX-RO connection unicity (two connections over RO must not have the same "
            + "id)");
        final JMXConnector jmxRoConnector2 = JMXConnectorFactory.connect(jmxRoServiceURL, env);
        assertFalse(
                "In case of multiple RO JMX connections the connectors must not provide the same connection "
    ...
    private void jmxClientHelper(RMAuthentication auth, Credentials adminCreds) throws IOException {
        ...
        assertTrue("Incorrect default behavior of connect() method it must use RMI protocol", client
                .getConnector().getConnectionId().startsWith("rmi"));
        client.disconnect();
        ...
        assertTrue("The helper connect(JMXTransportProtocol.RO) method must use RO protocol", client
                .getConnector().getConnectionId().startsWith("ro"));
        client.disconnect();
        assertFalse("The helper disconnect() must set the helper as disconnected", client.isConnected());
    }
}
