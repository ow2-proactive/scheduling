...
public abstract class InfrastructureManager implements Serializable {
	...
	public final void internalRegisterAcquiredNode(Node node) throws RMException {
		...
		synchronized (nodeAcquisitionLock) {
			...
			if (pn != null) {
				RMNodeEvent event = pn.createNodeEvent(RMEventType.NODE_REMOVED, pn.getState(),
	...
	protected final String addDeployingNode(String name, String command, String description, final long timeout) {
		...
		RMDeployingNode result = RMDeployingNodeAccessor.getDefault().newRMDeployingNode(name, nsStub, command,
				nsStub.getAdministrator(), description);
	...
	private void checkName(String name) {
		if (name.contains(" ")) {
			throw new IllegalArgumentException(
					"Deploying node name cannot contain white spaces: \"" + name + "\" is forbidden");
		}
		String pnURL = this.buildDeployingNodeURL(name);
		if (this.deployingNodes.containsKey(pnURL) || this.lostNodes.containsKey(pnURL)) {
			throw new IllegalArgumentException(RMDeployingNode.class.getSimpleName() + " with the same name (\"" + name
