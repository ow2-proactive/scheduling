			...
			try {
				...
				if (exitCode != 0 && !isJobIDValid) {
					...
					final String description = "SSH command failed to launch node on " + getBatchinJobSystemName()
							+ " scheduler" + lf + "   >Error code: " + exitCode + lf + "   >Errput: " + errput
							+ "   >Output: " + id;
					...
					handleWrongJobTermination(isJobIDValid, nodeName, dnURL, host, id, description, exitCode, submitCmd,
							deleteCmd);
				}
	...
	private void handleWrongJobTermination(final boolean isJobIDValid, final String nodeName, final String dnURL,
		...
		if (super.checkNodeIsAcquiredAndDo(nodeName, null,
			...
			if (isJobIDValid) {
				...
				logger.warn(
						"jobID " + id + " retrieved from SSH subprocess' output. Waiting for this node to register.");
			} else {
	...
	private void deleteJob(String jobID) throws RMException {
		...
		while (true) {
			try {
				...
				if (exitCode != 0) {
					logger.error(
							"Cannot delete job " + jobID + ". " + deleteCmd + " command returned != 0 -> " + exitCode);
					throw new RMException(
							"Cannot delete job " + jobID + ". " + deleteCmd + " command returned != 0 -> " + exitCode);
				} else {
