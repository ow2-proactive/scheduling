...
public class SSHInfrastructureV2 extends HostsFileBasedInfrastructureManager {
    ...
    @Configurable(description = "autoGenerated, useStartupScript or useNodeJarStartupScript. Please refer to the infrastructure documentation", sectionSelector = 1)
    protected String deploymentMode = "autoGenerated";

    @Configurable(description = "The URL from which the node.jar will be downloaded. Used only when useNodeJarStartupScript mode is selected", sectionSelector = 3)
    protected String nodeJarUrl = InitScriptGenerator.createNodeJarUrl();

    @Configurable(textArea = true, description = "Standard Startup script that will start the ProActive node agents. Used only when useStartupScript mode is selected", sectionSelector = 3)
    protected String startupScriptStandard = InitScriptGenerator.getDefaultLinuxStandardCommand();

    @Configurable(textArea = true, description = "Startup script that will download a Java Runtime Environment, a jar archive containing all ProActive libraries (node.jar) and start ProActive Agent using it. Used only when useNodeJarStartupScript mode is selected", sectionSelector = 3)
