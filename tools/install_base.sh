...
if [[ "$OLD_PADIR" == "" ]]; then
    ...
    if confirm "Do you want to enable automatic job/node history removal? [Y/n] " ; then
         ...
         sed "s/^pa\.job\.cancelled\.execution\.history\.maxperiod=.*/pa.job.cancelled.execution.history.maxperiod=$JOB_CLEANUP_SECONDS/g"  -i "$PA_ROOT/default/dist/war/job-planner/WEB-INF/classes/application.properties"
...
if which git > /dev/null 2>&1; then
    ...
    if [[ "$OLD_PADIR" != "" ]]; then
        ...
        if [[ -L $OLD_PADIR/logs ]]; then
            ln -s $(readlink -f $OLD_PADIR/logs) $PA_ROOT/default/logs
