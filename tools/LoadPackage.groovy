...
class LoadPackage {
    ...
    LoadPackage(binding) {
        ...
        this.SESSION_ID = binding.variables.get("pa.scheduler.session.id")
    ...
    def run(File package_dir) {
        ...
        def catalog_buckets_list = slurper.parseText(response.toString())
        package_buckets_list.each { bucket ->
            // Does the bucket already exist? -------------
            def bucket_found = catalog_buckets_list.find { object -> object.name == bucket }
            writeToOutput(" bucket_list " + bucket + " found? " + (bucket_found != null))

            // Create a bucket_list if needed -------------
            def bucket_id = null
            if (bucket_found) {
            ...
            catalog_map.get("objects").each { object ->
                ...
                // Push the object to the bucket_list if it not exists
