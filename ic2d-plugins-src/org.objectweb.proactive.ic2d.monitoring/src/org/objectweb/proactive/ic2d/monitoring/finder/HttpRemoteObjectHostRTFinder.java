...
//
			...
			URI[] remoteObjectUris = RemoteObjectHelper.getRemoteObjectFactory(Constants.XMLHTTP_PROTOCOL_IDENTIFIER).list(url);

			for (int i = 0 ; i < remoteObjectUris.length; i++) {
				RemoteObject rro =  RemoteObjectHelper.getRemoteObjectFactory(Constants.XMLHTTP_PROTOCOL_IDENTIFIER).lookup(remoteObjectUris[i]);
